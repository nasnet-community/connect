...
const SECURITY_RISK_THRESHOLD = parseInt(import.meta.env.VITE_SECURITY_RISK_THRESHOLD || "50", 10);
const ENABLE_VM_DETECTION = import.meta.env.VITE_ENABLE_VM_DETECTION !== "false";
const ENABLE_HEADLESS_DETECTION = import.meta.env.VITE_ENABLE_HEADLESS_DETECTION !== "false";
const ENABLE_AUTOMATION_DETECTION = import.meta.env.VITE_ENABLE_AUTOMATION_DETECTION !== "false";
const DETECTION_TIMEOUT = parseInt(import.meta.env.VITE_DETECTION_TIMEOUT || "1000", 10);
...
export function detectSuspiciousEnvironment(): {
  ...
  try {
    ...
    const vmFlags = [
      ...
      navigator.platform?.includes('Linux') && screen.width === 1024 && screen.height === 768,
    ...
    const automationFlags = [
      ...
      (window as any).chrome?.runtime && (window as any).chrome.runtime.onConnect?.hasListeners?.(),
    ];
