...
const DEBUG_MODE = import.meta.env.VITE_DEBUG_MODE === "true" || import.meta.env.NODE_ENV === "development";
const DEBUG_ITERATIONS = parseInt(import.meta.env.VITE_DEBUG_ITERATIONS || "5", 10);
const DEBUG_SAMPLES = parseInt(import.meta.env.VITE_DEBUG_SAMPLES || "3", 10);
const EXPOSE_TO_WINDOW = import.meta.env.VITE_EXPOSE_DEBUGGER === "true" && DEBUG_MODE;
...
export async function testUUIDConsistency(iterations: number = DEBUG_ITERATIONS): Promise<{
  uuids: string[];
  isConsistent: boolean;
  analysis: string;
}> {
...
export async function analyzeFingerprintStability(samples: number = DEBUG_SAMPLES): Promise<FingerprintAnalysis> {
  if (DEBUG_LOGGING) {
    console.log(`🔍 Analyzing fingerprint stability over ${samples} samples...`);
  }
  ...
  const uniquenessScore = uniqueFingerprints.size === 1 ? 100 : 
    Math.max(0, 100 - ((uniqueFingerprints.size - 1) * 20));
  
  // Calculate stability (how consistent components are)
  const stableComponents = Object.values(componentCounts).filter(count => count === samples).length;
  const stabilityScore = Math.round((stableComponents / Object.keys(componentCounts).length) * 100);
  ...
  if (uniquenessScore === 100) {
    ...
    analysis.push(`⚠️ Fingerprint varies across samples (${uniqueFingerprints.size} unique fingerprints)`);
  }
  ...
  if (stabilityScore > 90) {
    ...
    analysis.push("❌ Component stability is poor - fingerprinting may be unreliable");
  }
  
  if (totalComponents < 15) {
    analysis.push("⚠️ Fingerprint has fewer components than recommended for security");
...
export async function testSuspiciousEnvironmentDetection(): Promise<{
  ...
  const indicators = {
    ...
    commonVMResolution: navigator.platform.includes('Linux') && 
  ...
  if (indicators.webdriver) {
    recommendations.push("WebDriver property detected - likely automated browser");
  }
  if (indicators.zeroOuterDimensions) {
    recommendations.push("Zero outer window dimensions detected - likely headless browser");
  }
  if (indicators.noPlugins) {
    recommendations.push("No browser plugins detected - may indicate automation");
  }
  ...
  if (!isSuspicious) {
    recommendations.push("✅ Environment appears normal - no suspicious indicators detected");
  }
...
export async function runSecurityTests(): Promise<SecurityTestResults> {
  ...
  if (!uuidTest.isConsistent) {
    ...
    recommendations.push("UUID generation is inconsistent - check fingerprinting stability");
  }
  ...
  if (securityInfo.isFallback) {
    ...
    recommendations.push("Using fallback UUID generation - security may be reduced");
  }
  ...
  if (riskScore === 0) {
    ...
    recommendations.push("⚠️ Minor security concerns detected - system should work well");
  } else if (riskScore < 60) {
    recommendations.push("⚠️ Moderate security concerns - may experience some restrictions");
  } else {
...
- Active Indicators: ${Object.entries(suspiciousTest.indicators)
    .filter(([, value]) => value)
    .map(([key]) => key)
    .join(', ') || 'None'}
