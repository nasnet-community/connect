...
export const onGet: RequestHandler = async (requestEvent) => {
  ...
  try {
    ...
    if (!isAuthenticated) {
      json(401, { error: 'Unauthorized. Please log in to access this resource.' });
      return;
    }
    ...
    if (format === 'csv') {
      ...
      headers.set('Content-Disposition', `attachment; filename="vpn-credentials-${filter || 'all'}-${dateStr}.csv"`);
