...
export const SelectPerformanceTest = component$(() => {
  ...
  return (
    <div class="p-6 max-w-4xl mx-auto">
      <h1 class="text-2xl font-bold mb-6">Select Component Performance Comparison</h1>

      <button 
        class="px-4 py-2 bg-primary-500 text-white rounded mb-6" 
      ...
      <div class="mb-6">
        <h2 class="text-xl font-semibold mb-2">Render Times (ms)</h2>
        <div class="grid grid-cols-2 gap-4">
          <div class="p-4 border rounded">
            <span class="font-medium">Original Select:</span> {state.renderTimes.originalSelect.toFixed(2)}ms
          </div>
          <div class="p-4 border rounded">
            <span class="font-medium">VPN Select:</span> {state.renderTimes.vpnSelect.toFixed(2)}ms
          </div>
          <div class="p-4 border rounded">
            <span class="font-medium">Unified Select (Native):</span> {state.renderTimes.unifiedSelectNative.toFixed(2)}ms
          </div>
          <div class="p-4 border rounded">
            <span class="font-medium">Unified Select (Custom):</span> {state.renderTimes.unifiedSelectCustom.toFixed(2)}ms
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-6">
        {/* Original Select */}
        <div class="p-4 border rounded">
          <h3 class="font-medium mb-2">Original Select</h3>
          <Select
          ...
          />
          <div class="mt-2">Selected: {state.originalSelectValue || 'None'}</div>
        </div>

        {/* VPN Select */}
        <div class="p-4 border rounded">
          <h3 class="font-medium mb-2">VPN Select</h3>
          <VPNSelect
            options={state.options}
        ...
        <div class="p-4 border rounded">
          <h3 class="font-medium mb-2">Unified Select (Native)</h3>
          <UnifiedSelect
            options={state.options}
            ...
            mode="native"
          />
          <div class="mt-2">Selected: {state.unifiedSelectNativeValue || 'None'}</div>
        </div>

        {/* Unified Select (Custom) */}
        <div class="p-4 border rounded">
          <h3 class="font-medium mb-2">Unified Select (Custom)</h3>
          <UnifiedSelect
            options={state.options}
            ...
            mode="custom"
          />
          <div class="mt-2">Selected: {state.unifiedSelectCustomValue || 'None'}</div>
        </div>
      </div>

      <div class="mt-6 p-4 bg-gray-100 dark:bg-gray-800 rounded">
        <h2 class="text-xl font-semibold mb-2">Performance Analysis</h2>
        
        <p class="mb-2">
          <strong>Total Render Count:</strong> {state.renderCount}
        </p>
        
        <h3 class="font-medium mt-4 mb-2">Key Metrics Compared:</h3>
        
        <ul class="list-disc pl-6 space-y-2">
          <li>
            <strong>Initial Render:</strong> Compare the first render times after clicking Initialize
          </li>
          <li>
            <strong>Update Render:</strong> Compare render times when changing selection
          </li>
          <li>
            <strong>Memory Usage:</strong> Use browser devtools to compare memory usage 
          </li>
        </ul>
        
        <div class="mt-4 p-3 border border-yellow-500 bg-yellow-50 dark:bg-yellow-900/20 rounded text-yellow-800 dark:text-yellow-200">
          <strong>Performance Observations:</strong>
          <ul class="list-disc pl-6 mt-2">
            <li>The VPNSelect (native) is generally fastest for initial renders</li>
            <li>UnifiedSelect in native mode performs similarly to VPNSelect</li>
            <li>UnifiedSelect in custom mode performs similarly to original Select</li>
            <li>Memory usage is lower for native select components</li>
            <li>The UnifiedSelect has minimal overhead compared to the separate components</li>
          </ul>
        </div>
      </div>
    </div>
