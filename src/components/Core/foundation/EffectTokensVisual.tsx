...
export const ShadowSample = component$<{
  ...
  return (
    <div class="mb-6 p-4">
      <div class={`${name} bg-white dark:bg-neutral-800 p-6 rounded-lg w-full h-32 flex items-center justify-center`}>
        <div>
          <div class="font-medium">{name}</div>
          <div class="text-sm text-neutral-500 dark:text-neutral-400 max-w-xs text-center">{description}</div>
        </div>
      </div>
    </div>
  );
...
export const BorderRadiusSample = component$<{
  ...
  return (
    <div class="mb-6">
      <div class={`${name} bg-primary-500 h-24 w-full flex items-center justify-center text-white font-medium`}>
        <div>
          <div>{name}</div>
...
export const AnimationSample = component$<{
  ...
  return (
    <div class="mb-6 p-2">
      <div 
        class="bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg p-4 flex flex-col items-center"
      >
        <div 
          class={`w-16 h-16 bg-primary-500 rounded-md flex items-center justify-center text-white mb-4 ${isAnimating.value ? name : ''}`}
          onClick$={() => {
        ...
        <div class="font-medium">{name}</div>
        <div class="text-sm text-neutral-500 dark:text-neutral-400 text-center mt-1">{description}</div>
        <button 
          class="mt-3 px-3 py-1.5 bg-primary-100 dark:bg-primary-900 text-primary-800 dark:text-primary-200 rounded text-sm font-medium"
          onClick$={() => {
            isAnimating.value = false;
            setTimeout(() => {
              isAnimating.value = true;
...
export const TransitionSample = component$<{
  ...
  return (
    <div class="mb-6 p-2">
      ...
      >
        <div 
          class={`bg-secondary-500 rounded-md flex items-center justify-center text-white mb-4 transition-all duration-300 ease-in-out ${name}`}
          style={{ 
        ...
        <div class="text-sm text-neutral-500 dark:text-neutral-400 text-center mt-1">{description}</div>
        <button 
          class="mt-3 px-3 py-1.5 bg-secondary-100 dark:bg-secondary-900 text-secondary-800 dark:text-secondary-200 rounded text-sm font-medium"
          onClick$={() => {
...
export const EffectTokensVisual = component$(() => {
  ...
  return (
    <div class="space-y-12">
      <section>
        <h2 class="text-2xl font-semibold mb-6">Shadow Tokens</h2>
      ...
      <section>
        <h2 class="text-2xl font-semibold mb-6">Border Radius Tokens</h2>
      ...
      <section>
        <h2 class="text-2xl font-semibold mb-6">Directional Border Radius</h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-6">
          <BorderRadiusSample name="rounded-t-lg" value="Top corners: 0.5rem (8px)" />
          <BorderRadiusSample name="rounded-r-lg" value="Right corners: 0.5rem (8px)" />
          <BorderRadiusSample name="rounded-b-lg" value="Bottom corners: 0.5rem (8px)" />
          <BorderRadiusSample name="rounded-l-lg" value="Left corners: 0.5rem (8px)" />
          <BorderRadiusSample name="rounded-tl-lg" value="Top-left corner: 0.5rem (8px)" />
          <BorderRadiusSample name="rounded-tr-lg" value="Top-right corner: 0.5rem (8px)" />
          <BorderRadiusSample name="rounded-bl-lg" value="Bottom-left corner: 0.5rem (8px)" />
          <BorderRadiusSample name="rounded-br-lg" value="Bottom-right corner: 0.5rem (8px)" />
        </div>
      </section>

      <section>
        <h2 class="text-2xl font-semibold mb-6">Animation Tokens</h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          <AnimationSample 
      ...
      <section>
        <h2 class="text-2xl font-semibold mb-6">Transition Properties</h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-6">
          <TransitionSample 
            name="transition" 
      ...
      <div class="p-6 border border-neutral-200 rounded-lg dark:border-neutral-700 mt-8">
        ...
        <p class="mb-3">This component provides a visual reference for effect tokens (shadows, border radius, animations, transitions) in the design system. It can be imported and used in documentation or Storybook.</p>
