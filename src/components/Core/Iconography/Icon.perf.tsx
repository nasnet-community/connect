...
export const IconPerformanceTest = component$(() => {
  ...
  return (
    <div class="p-4">
      <h2 class="text-xl font-bold mb-4">Icon Performance Test</h2>
      
      <div class="mb-8">
        <h3 class="text-lg font-semibold mb-2">Basic Icons (13)</h3>
        <div class="flex flex-wrap gap-2">
          {icons.map((icon, index) => (
            <Icon key={`basic-${index}`} icon={icon} />
          ))}
        </div>
      </div>
      
      <div class="mb-8">
        <h3 class="text-lg font-semibold mb-2">Many Icons (130)</h3>
        <div class="flex flex-wrap gap-2">
          {manyIcons.map((icon, index) => (
            <Icon key={`many-${index}`} icon={icon} size="sm" />
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <ul class="list-disc list-inside space-y-2">
          <li>The Icon component renders efficiently by using CSS classes for styling</li>
          <li>Icon size is controlled via Tailwind CSS classes rather than inline styles</li>
          <li>Using <code>useStyles$()</code> ensures styles are only injected once</li>
          <li>Icons are lazy-loaded via Qwik's <code>$()</code> function, ensuring they're only loaded when needed</li>
          <li>The component avoids unnecessary re-renders by using a constant mapping for size and color classes</li>
        </ul>
      </div>
    </div>
  );
