...
export function useDialog(params: UseDialogParams): UseDialogReturn {
  ...
  const initialIsOpen = typeof propIsOpen === 'boolean' ? propIsOpen : 
                        (openSignal ? openSignal.value : false);
  ...
  useOnDocument(
    ...
    $((event: KeyboardEvent) => {
      ...
      if (trapFocus && isOpenSignal.value && event.key === 'Tab' && focusableElements.value.length > 0) {
