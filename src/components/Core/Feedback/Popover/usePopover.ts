...
export const PopoverContext = createContextId<PopoverState>('connect.popover-context');
...
export function usePopover(props: PopoverProps): UsePopoverReturn {
  ...
  // Set up signals
  const isOpenSignal = useSignal<boolean>(propIsOpen === true || (openSignal && openSignal.value === true) || false);
