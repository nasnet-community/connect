import { component$, useStore, $, useVisibleTask$, useContextProvider, createContextId } from '@builder.io/qwik';
import type { ToastContainerProps, ToastProps, ToastService, ToastOptions, ToastPosition } from './Toast.types';
import { Toast } from './Toast';

/**
 * Context for the toast service
 */
export const ToastServiceContext = createContextId<ToastService>('toast-service');
...
export const ToastContainer = component$<ToastContainerProps>(({
  ...
  const toastService: ToastService = {
    ...
    show: $(async ({ id, ...options }: ToastOptions): Promise<string> => {
      const toastId = id || `toast-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
    ...
    // Show an info toast
    info: $(async (message: string, options?: Partial<ToastOptions>): Promise<string> => {
    ...
    success: $(async (message: string, options?: Partial<ToastOptions>): Promise<string> => {
      return toastService.show({
        message,
        status: 'success',
        ...options,
    ...
    warning: $(async (message: string, options?: Partial<ToastOptions>): Promise<string> => {
      return toastService.show({
        message,
        status: 'warning',
        ...options,
    ...
    // Show an error toast
    error: $(async (message: string, options?: Partial<ToastOptions>): Promise<string> => {
      return toastService.show({
        message,
        status: 'error',
        ...options,
    ...
    loading: $(async (message: string, options?: Partial<ToastOptions>): Promise<string> => {
      return toastService.show({
        message,
        loading: true,
        persistent: true,
  ...
  useVisibleTask$(({ cleanup }) => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.altKey && event.key === 't' && sortedToasts.items.length > 0) {
        const toastElement = document.getElementById(sortedToasts.items[0].id);
        if (toastElement) {
          toastElement.focus();
        }
      }
      ...
      if (event.key === 'Escape' && document.activeElement?.getAttribute('role') === 'status') {
