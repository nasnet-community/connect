...
export const Slider = component$<SliderProps>((props) => {
  ...
  const { onChange$, onChangeEnd$, type, value, defaultValue, ...htmlProps } = rest;
  ...
  const styles = useSliderStyles({
    ...
    valueToPercent: (value: number) => utilities.valueToPercent(value) as unknown as number
  });
  ...
  useTask$(async ({ track }) => {
    ...
    if (isRangeType) {
      startValueText.value = await utilities.formatLabel(state.startValue.value);
      endValueText.value = await utilities.formatLabel(state.endValue.value);
    } else {
      singleValueText.value = await utilities.formatLabel(state.singleValue.value);
    }
  });
  ...
  useTask$(async ({ track }) => {
    ...
    const singlePercent = await utilities.valueToPercent(state.singleValue.value);
        ...
        {isRangeType ? (
          <>
            ...
            <RangeThumb
              ...
              constraints={{
                min,
                max: state.endValue.value - (isRangeType ? (props.minRange || 0) : 0)
              }}
            ...
            <RangeThumb
              ...
              maxValue={max}
              constraints={{
                min: state.startValue.value + (isRangeType ? (props.minRange || 0) : 0),
                max
              }}
              minRange={isRangeType ? props.minRange : undefined}
              disabled={props.disabled}
