...
import { useSliderUtilities } from './useSliderUtilities';
...
export function useSliderEvents({ props, state, utilities, orientation }: UseSliderEventsProps) {
  ...
  const handleThumbMouseDown = $((event: MouseEvent, thumb: 'single' | 'start' | 'end') => {
    if (disabled || readonly) return;
  ...
  const handleThumbKeyDown = $((event: KeyboardEvent, thumb: 'single' | 'start' | 'end') => {
    if (disabled || readonly) return;
