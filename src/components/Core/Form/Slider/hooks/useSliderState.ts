...
export function useSliderState({ props }: UseSliderStateProps) {
  ...
  const singleOnChangeEnd$ = !isRangeSlider(props) ? props.onChangeEnd$ : undefined;
  const rangeOnChangeEnd$ = isRangeSlider(props) ? props.onChangeEnd$ : undefined;
  ...
  const singleValueInit = propValue !== undefined && !isRange 
    ? propValue as number 
    : (!isRange && propDefaultValue !== undefined) 
      ? propDefaultValue as number 
      : min;
      
  const rangeStartValueInit = isRange && propValue !== undefined 
    ? (propValue as [number, number])[0] 
    : isRange && propDefaultValue !== undefined 
      ? (propDefaultValue as [number, number])[0] 
      : min;
      
  const rangeEndValueInit = isRange && propValue !== undefined 
  ...
  const effectiveState = useSignal<'default' | 'error' | 'success' | 'warning'>(
    ...
    initialSuccessMessage ? 'success' : 
    initialWarningMessage ? 'warning' : 
    'default'
  );
  ...
  const updateStartValue = $((value: number) => {
    ...
    const maxEndValue = maxRange !== undefined
      ? Math.min(endValue.value, min + maxRange)
      : endValue.value;
    
    // Ensure start value doesn't exceed end value minus the minimum range
    const constrainedValue = Math.max(min, Math.min(maxEndValue - minRange, value));
  ...
  const updateEndValue = $((value: number) => {
    ...
    const minStartValue = maxRange !== undefined
      ? Math.max(startValue.value, max - maxRange)
      : startValue.value;
    
    // Ensure end value doesn't fall below start value plus the minimum range
    const constrainedValue = Math.min(max, Math.max(minStartValue + minRange, value));
