...
import type { FormOptions, FormFieldOptions, FormValidationRule, FormState } from '../Form.types';
...
export function useForm(options: FormOptions = {}) {
  ...
  const updateFormValidationStatus = $(() => {
    ...
    const isValidating = Object.values(state.fields).some((field) => field.validating);
    
    state.isValid = !hasErrors;
    state.isValidating = isValidating;
    state.status = isValidating ? 'validating' : hasErrors ? 'invalid' : 'valid';
  });
  ...
  const setFieldValue = $((name: string, value: any) => {
    ...
    // Validate if needed
    const shouldValidate = (formOptions.validateOnChange ?? true) ||
  ...
  const validateField = $(async (name: string): Promise<string | undefined> => {
    ...
    if (!error && fieldOptions && ('validate' in fieldOptions) && fieldOptions.validate) {
  ...
  const resetForm = $(() => {
    ...
    Object.keys(state.fields).forEach((name) => {
      const initialValue = formOptions.initialValues?.[name] !== undefined 
        ? formOptions.initialValues[name] 
        : '';
  ...
  const handleSubmit$ = $(async (e?: SubmitEvent, onSubmitFn?: (values: Record<string, any>) => void | Promise<void>) => {
