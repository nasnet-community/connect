...
export interface UseFileValidationOptions {
  ...
  formatText$: QRL<(template: string, replacements: Record<string, string | number>) => string>;
}
...
export function useFileValidation(options: UseFileValidationOptions) {
  ...
  const validateFile = $((file: File): { valid: boolean; error?: string } => {
    if (maxFileSize && file.size > maxFileSize) {
      ...
      const errorMessage = formatText$(maxFileSizeExceededText, { maxSize: formattedSize }) as unknown as string;
    ...
    if (minFileSize && file.size < minFileSize) {
      ...
      const errorMessage = formatText$(minFileSizeText, { minSize: formattedSize }) as unknown as string;
