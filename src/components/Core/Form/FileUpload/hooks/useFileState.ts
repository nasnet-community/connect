...
export function useFileState(options: UseFileStateOptions) {
  ...
  const processFiles = $((fileList: FileList | File[], processOptions: ProcessFilesOptions): void => {
    if (disabled) return;
    ...
    for (const file of fileArray) {
      const validation = validateFile(file) as unknown as { valid: boolean; error?: string };
    ...
    if (onFilesSelected$ && newFiles.length > 0) {
      onFilesSelected$(newFiles.map(fileInfo => fileInfo.file as unknown as File));
    }
  });
  ...
  const updateFileStatus = $((fileId: string, updates: Partial<FileInfo>): void => {
