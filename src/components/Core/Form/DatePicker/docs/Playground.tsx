...
export default component$(() => {
  ...
  const selectedRange = useSignal<DateRange | undefined>({ startDate: null, endDate: null });
      ...
      <div class="w-full max-w-md mx-auto">
        ...
        <div class="mt-4 p-2 bg-surface-secondary dark:bg-surface-dark-secondary rounded text-sm">
          {props.mode === 'range' && selectedRange.value ? (
            <p>
              Selected range: {selectedRange.value.startDate ? new Date(selectedRange.value.startDate).toLocaleDateString() : 'None'} - 
              {selectedRange.value.endDate ? new Date(selectedRange.value.endDate).toLocaleDateString() : 'None'}
            </p>
          ) : (
            <p>
              Current selection: {selectedDate.value ? new Date(selectedDate.value).toLocaleDateString() : 'None'}
            </p>
          )}
        </div>
      </div>
