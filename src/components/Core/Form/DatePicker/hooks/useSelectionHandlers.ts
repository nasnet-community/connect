...
export interface UseSelectionHandlersResult {
  ...
  handleTimeSelect$: QRL<(hours: number, minutes: number, seconds?: number) => void>;
...
export function useSelectionHandlers(
  ...
  const handleDateSelect$ = $((date: Date) => {
    ...
    if (props.mode === 'single') {
      ...
      const rangeProps = props as RangeDatePickerProps;
      const currentRange = rangeProps.value || { startDate: null, endDate: null };
      ...
      const currentDateTime = dateTimeProps.value ? new Date(dateTimeProps.value) : new Date();
