...
export const CalendarDay = component$<CalendarDayProps>(({
  ...
  const isDisabled = dateUtils.isDateDisabled(date, minDate, maxDate, disabledDates);
  ...
  if (selected) {
    if (selected instanceof Date) {
      ...
      isSelected = dateUtils.isSameDay(date, startDate) || dateUtils.isSameDay(date, endDate);
    }
  }
  ...
  const dayClasses = [
    ...
    'transition-colors',
    'duration-150',
    isCurrentMonth ? 'text-text-primary dark:text-text-dark-primary' : 'text-text-muted dark:text-text-dark-muted',
    isToday ? 'border border-primary-500' : '',
    isSelected ? 'bg-primary-500 text-white' : '',
    isInRange && !isSelected ? 'bg-primary-100 dark:bg-primary-800' : '',
    isDisabled ? 'cursor-not-allowed opacity-50' : 'hover:bg-primary-100 dark:hover:bg-primary-800',
  ].filter(Boolean).join(' ');
