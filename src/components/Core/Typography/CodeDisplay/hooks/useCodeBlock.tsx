...
export function useCodeBlock({
  ...
  highlightLines,
  theme = "system",
}: Pick<CodeBlockProps, "code" | "language" | "showLineNumbers" | "highlight" | "highlightLines" | "theme">) {
  ...
  useVisibleTask$(({ track }) => {
    ...
    import('highlight.js/lib/core')
      .then(async (hljs) => {
        ...
        if (!highlight) {
          highlightedCode.value = processCodeWithoutHighlight(code, showLineNumbers);
          return;
        }
        
        // Determine which language to load
        const langName = language === 'js' ? 'javascript' 
        ...
        try {
          ...
          // Highlight the code
          let highlighted = hljs.default.highlight(code, { language: langName }).value;
          ...
          console.warn(`Language '${langName}' not supported for syntax highlighting`);
          highlightedCode.value = processCodeWithoutHighlight(code, showLineNumbers);
        }
      })
      .catch(() => {
...
// Helper functions
export function processCodeWithoutHighlight(code: string, showLineNumbers: boolean): string {
  ...
  if (showLineNumbers) {
    ...
    processedCode = lines.map(line => `<span class="hljs-ln">${line}</span>`).join('\n');
  }
  
  return processedCode;
}

export function processLineHighlights(code: string, highlightLines?: string): string {
...
export function processLineNumbers(code: string, showLineNumbers: boolean): string {
  if (!showLineNumbers) return code;
  
  const lines = code.split('\n');
  const numberedLines = lines.map(line => `<span class="hljs-ln">${line}</span>`);
  return numberedLines.join('\n');
}

export function parseHighlightRanges(highlightLines?: string): Array<{start: number, end: number}> {
