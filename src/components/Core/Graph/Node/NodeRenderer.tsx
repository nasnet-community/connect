  ...
  try {
    ...
    return (
      <g transform={`translate(${node.x}, ${node.y})`}>
        ...
        {/* Node label with dark mode support */}
        <text 
          x="0" 
          y={nodeSize + 15}
          text-anchor="middle" 
        ...
        {/* Add an overlaid shadow text for better readability on dark backgrounds */}
        <text 
          x="0" 
          y={nodeSize + 15}
          text-anchor="middle" 
          ...
          font-size="11" 
          font-weight="bold"
          class="node-label-shadow dark:inline hidden"
          style={{
            opacity: 0.3,
...
function renderBasicNode(node: GraphNode, color?: string) {
  ...
  return (
    <g transform={`translate(${node.x}, ${node.y})`}>
      ...
      <circle
        r={nodeSize}
        fill={`${node.color || color || '#fbbf24'}33`}
        class="node-highlight"
      />
