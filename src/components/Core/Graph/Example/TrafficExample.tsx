...
export const NetworkTrafficExample = component$(() => {
  ...
  const nodes = [
    ...
    createNode("DomesticWebsite", "website", 700, 50, { label: "Local Website" })
  ];
  ...
  const trafficTypes = [
    { type: "Domestic", color: "#84cc16", description: "Traffic to/from local networks" },
    { type: "Foreign", color: "#9333ea", description: "Traffic to/from external/international networks" },
  ...
  return (
    <div class="p-6 bg-white dark:bg-slate-800 rounded-lg">
      <h2 class="text-xl font-semibold mb-4 text-slate-800 dark:text-white">Traffic Type Explorer</h2>
      
      {/* Traffic type toggles - interactive legend */}
      <div class="mb-6 flex flex-wrap gap-3">
        {trafficTypes.map(({ type, color }) => (
          <button
            ...
            class={{
              "flex items-center gap-2 px-3 py-2 rounded-lg border transition-all": true,
              "bg-opacity-20 shadow-sm": activeTrafficTypes.value[type],
              "opacity-50": !activeTrafficTypes.value[type]
            }}
            style={{ 
              backgroundColor: activeTrafficTypes.value[type] ? `${color}20` : 'transparent',
              borderColor: color
            }}
          >
            <span 
      ...
      {selectedConnection.value && (
        <div class="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-md">
          ...
          <p class="text-xs text-slate-600 dark:text-slate-300">
            Type: {selectedConnection.value.trafficType || 'Standard'} | 
            From: {selectedConnection.value.from} | 
            To: {selectedConnection.value.to}
          </p>
        </div>
      )}
        ...
        {trafficTypes.map(({ type, color, description }) => (
          ...
          >
            ...
            <div class="mt-2 text-xs text-slate-500 dark:text-slate-400">
              ...
              {type === "Domestic" && "Features: Animated packets flowing to local sites"}
              {type === "Foreign" && "Features: Animated packets flowing to external sites"}
            </div>
          </div>
        ))}
      </div>
