...
import { connectionColors, connectionStyles } from "../Connection/ConnectionUtils";
...
export const ConnectionTypeExample = component$(() => {
  ...
  const getSelectedStyleProps = () => {
    ...
    const style = connectionStyles[selectedConnectionType.value] || { dashed: false, width: 2 };
    return {
  ...
  const getConnectionColor = (type: string): string => {
    ...
    const validType = (Object.keys(connectionColors).includes(type)) ? 
      type as ConnectionType : undefined;
  ...
  const getConnectionStyle = (type: string) => {
    ...
    return validType ? connectionStyles[validType] : { dashed: false, width: 2 };
  };
  ...
  const getConnectionDetails = (connType: string) => {
    return connectionTypeDetails.find(c => c.type === connType) || {
      type: connType,
  ...
  return (
    <div class="p-6 bg-white dark:bg-slate-800 rounded-lg">
      <h2 class="text-xl font-semibold mb-2 text-slate-800 dark:text-white">Connection Types Explorer</h2>
      <p class="mb-4 text-sm text-slate-600 dark:text-slate-300">
        Visualize the different physical connection types available for network graphs
      </p>
      
      {/* Connection Type Selector */}
      <div class="mb-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2">
        {connectionTypeDetails.map(({ type }) => {
          ...
          return (
            <button
              ...
              class={{
                "p-3 border rounded-md transition-all text-center": true,
                "shadow-md": selectedConnectionType.value === type,
                "opacity-70": selectedConnectionType.value !== type
              }}
              style={{ 
                borderColor: color,
                backgroundColor: selectedConnectionType.value === type 
                  ? `${color}15` 
                  : 'transparent'
              }}
            >
      ...
      {showDetailedView.value && selectedConnectionType.value && (
        <div 
          ...
          style={{ borderColor: getConnectionColor(selectedConnectionType.value) }}
        >
          <div class="flex flex-wrap items-center gap-4 mb-3">
            <h3 class="text-lg font-semibold">{selectedConnectionType.value} Connection</h3>
            <div class="flex items-center gap-2">
              <div 
                class="inline-block h-1 w-8 rounded-full"
                style={{ 
                  backgroundColor: getConnectionColor(selectedConnectionType.value),
                  height: `${getConnectionStyle(selectedConnectionType.value).width || 2}px`,
                  borderStyle: getConnectionStyle(selectedConnectionType.value).dashed ? 'dashed' : 'solid'
                }}
              ></div>
              <span class="text-xs text-slate-500">
                {getSelectedStyleProps().dashed} line, {getSelectedStyleProps().width} width
              </span>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
            <div class="p-2 bg-slate-50 dark:bg-slate-700 rounded">
              <span class="font-medium">Typical Speed:</span> {getConnectionDetails(selectedConnectionType.value).speed}
            </div>
            <div class="p-2 bg-green-50 dark:bg-green-900/20 rounded">
              <span class="font-medium">Pros:</span> {getConnectionDetails(selectedConnectionType.value).pros}
            </div>
            <div class="p-2 bg-amber-50 dark:bg-amber-900/20 rounded">
              <span class="font-medium">Cons:</span> {getConnectionDetails(selectedConnectionType.value).cons}
            </div>
          </div>
        </div>
      )}
      ...
      <div class="mt-6 p-4 border border-slate-200 dark:border-slate-700 rounded-lg">
        <h3 class="font-semibold mb-2 text-slate-800 dark:text-white">Connection Type Features</h3>
        <ul class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-1 text-sm text-slate-600 dark:text-slate-300">
