...
export const TimePicker = component$<TimePickerProps>(
  ({
    ...
    const variantClasses: Record<
      string,
      { wrapper: string; select: string; separator: string }
    > = {
      default: {
        wrapper: "",
        select: `
  ...
  `;
    ...
    return (
      <div
        class={`${inline ? "inline-block" : "block"} ${sizeClasses[size].wrapper} ${className || ""}`}
      >
        {label && (
          <label
            for={id}
            class={`
            text-text-DEFAULT mb-2 flex items-center gap-2
            font-medium dark:text-text-dark-default
        ...
        <div class={`flex items-center ${sizeClasses[size].container}`}>
          ...
          <div class="flex-1 sm:flex-initial">
            <label
              class={`
              mb-1.5 block font-medium text-gray-600
              dark:text-gray-400 sm:sr-only
          ...
          {showClearButton && onClear$ && (
            <div class="ml-2 flex items-center">
              <button
                ...
                disabled:cursor-not-allowed disabled:opacity-50 dark:text-gray-500
        ...
        {/* Loading Indicator */}
        {loading && (
          <div class="bg-surface-DEFAULT/50 absolute inset-0 flex items-center justify-center rounded-lg dark:bg-surface-dark/50">
