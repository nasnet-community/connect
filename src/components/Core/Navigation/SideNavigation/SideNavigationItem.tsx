...
import type { SideNavigationItem, SideNavigationProps } from './SideNavigation.types';
...
export const SideNavigationItemComponent = component$<SideNavigationItemComponentProps>((props) => {
  const { 
    item, 
    isCollapsed, 
    depth = 0, 
  ...
  const iconContent = item.icon ? (
    <span class={`flex-shrink-0 ${isCollapsed ? 'mx-auto' : 'mr-3'} ${iconSizeClass.value}`}>
      {item.icon}
    </span>
  ) : null;
  ...
  const badgeContent = !isCollapsed && item.badge ? (
    <span class="ml-auto flex-shrink-0 text-xs">
      {item.badge}
    </span>
  ) : null;
  
  const expansionIndicator = !isCollapsed && hasItems ? (
    <span class={`ml-auto flex-shrink-0 transition-transform duration-200 ${isExpanded.value ? 'rotate-90' : ''}`}>
      <svg 
