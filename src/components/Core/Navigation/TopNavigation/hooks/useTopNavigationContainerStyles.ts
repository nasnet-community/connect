...
export function useTopNavigationContainerStyles(props: UseTopNavigationContainerStylesProps) {
  ...
  const variantClass = useComputed$(() => {
    ...
    if (variant === 'minimal') return 'bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800';
    if (variant === 'filled') return 'bg-primary-600 text-white shadow';
    if (variant === 'bordered') return 'bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 shadow-sm';
