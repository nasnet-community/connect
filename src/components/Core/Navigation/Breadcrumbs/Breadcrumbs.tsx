...
const Breadcrumbs = component$<BreadcrumbsProps>((props) => {
  ...
  const { displayedItems } = useDisplayedItems(items, maxItems, expanderLabel, screenWidth);
  ...
  return (
    <nav 
      aria-label={label}
      {...rest}
      class={`text-sm ${props.class || ''}`}
    >
      <ol 
        class={`flex flex-wrap items-center ${isRtl.value ? 'space-x-reverse' : ''}`}
