...
export interface TabItemProps {
  ...
  showIcons: boolean;
  fullWidth: boolean;
  sizeClasses: Record<TabSize, {
    tab: string;
    icon: string;
    count: string;
  }>;
  variantClasses: Record<TabVariant, {
    container: string;
    list: string;
    tab: {
      base: string;
...
export const TabItem = component$<TabItemProps>(({
  ...
  return (
    <li 
      class={`${widthClass} ${variant === "underline" ? "mr-2" : ""}`}
      role="presentation"
    >
      <button
      ...
      >
        <span class="flex items-center">
          {showIcons && tab.icon && (
            <span class={sizeClasses[size].icon}>
              {tab.icon}
            </span>
          )}
          ...
          {typeof tab.count === "number" && (
            <span class={`
              ...
              rounded-full bg-surface-secondary px-1.5 py-0.5 text-center font-medium
              dark:bg-surface-dark-secondary
              ${isActive ? "bg-primary-100 text-primary-800 dark:bg-primary-900/30 dark:text-primary-300" : ""}
            `}>
              {tab.count}
            </span>
          )}
