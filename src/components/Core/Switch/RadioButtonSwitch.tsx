...
export const RadioButtonSwitch = component$<RadioButtonSwitchProps>(({
  ...
  return (
    <label
      class={`inline-flex items-center cursor-pointer ${disabled ? "opacity-50 cursor-not-allowed" : ""} ${className}`}
    >
      <div class="relative">
        <input
          type="checkbox"
        ...
        <div
          class={`${sizeClasses} rounded-full transition-colors duration-200 ease-in-out bg-gray-200 dark:bg-gray-700 ${
            isChecked.value ? "bg-primary-500 dark:bg-primary-600" : ""
          }`}
        ></div>
        <div
          class={`absolute top-0.5 left-0 ${thumbSizeClasses} bg-white rounded-full transition-transform duration-200 ease-in-out transform ${thumbPositionClasses}`}
        ></div>
      </div>
