...
export interface UseTableReturn<T = any> {
  ...
  handleSort$: QRL<(columnId: string) => void>;
  handleRowClick$: QRL<(row: T, index: number, event: MouseEvent) => void>;
...
export function useTable<T = any>(props: TableProps<T>): UseTableReturn<T> {
  ...
  const handleSort$ = $((columnId: string) => {
    ...
    const currentDirection = currentSort.value.column === columnId 
    ...
    // Update controlled sortState if provided
    if (sortState && 'value' in sortState) {
  ...
  const getRowId = (row: T, index: number): string => {
    if (rowId) {
      // Wrap in a try-catch to handle async functions and ensure we return a string
      try {
        const result = rowId(row, index);
        // Handle returned promise (shouldn't happen with proper typing)
        if (result instanceof Promise) {
          console.warn('rowId function returned a Promise, using index as fallback');
          return `row-${index}`;
        }
...
export function getCellValue<T>(row: T, column: TableColumn<T>, rowIndex: number): any {
