...
/**
 * Table component for displaying tabular data with data and columns
 */
...
/**
 * SimpleTable component for compositional usage with children
 */
export const SimpleTable = component$<SimpleTableProps>((props) => {
  const {
    caption,
    variant = "default",
    size = "md",
  ...
  // Container classes for compositional mode
  const containerClasses = [
    horizontalScroll ? "overflow-x-auto" : "",
    className,
  ]
    ...
    <div class={containerClasses} style={containerStyle}>
      <table class={tableClasses} id={id}>
        ...
        {pagination && (
          <tfoot>
            <tr>
              <td class="px-4 py-3">{pagination}</td>
            </tr>
          </tfoot>
        )}
      </table>
...
function renderTableCell(
  ...
  const cellClasses = [
    ...
    // Fixed columns with RTL support
    column.fixed === "left"
      ? "sticky start-0 z-10"
      : column.fixed === "right"
        ? "sticky end-0 z-10"
