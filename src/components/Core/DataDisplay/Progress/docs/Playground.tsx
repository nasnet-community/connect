...
export default component$(() => {
  ...
  const generateProgressBarCode = $(() => {
    ...
    if (progressBarState.valuePosition !== 'right' && progressBarState.showValue) {
  ...
  const generateSpinnerCode = $(() => {
    ...
    spinnerCode.value = props.length ? `<Spinner ${props.join(' ')} />` : '<Spinner />';
  });
  ...
  return (
    <PlaygroundTemplate
      ...
      preview={
        <div class="p-4 flex justify-center items-center min-h-[150px]">
          {activeComponent.value === 'progressbar' ? (
      ...
      code={activeComponent.value === 'progressbar' ? progressBarCode : spinnerCode}
      onMount$={() => {
        generateProgressBarCode();
        generateSpinnerCode();
      }}
    >
      <p>
        Use the controls above to customize the selected progress component. Toggle between
        ProgressBar and Spinner to explore the different options available for each component.
        The code will update automatically based on your selections, allowing you to see how
        different props affect the appearance and behavior of the components.
      </p>
    </PlaygroundTemplate>
  );
});
