...
const SpinnerLabel = component$<{
  ...
  return (
    <span class={`${spacingClass} ${labelClass}`}>
      {label}
    </span>
  );
});
...
const BorderSpinner = component$<{
  ...
  const { sizeClasses, colorClasses, animationDuration, className, ariaLabel } = props;
  
  return (
    <div
      class={`inline-block rounded-full animate-spin ${sizeClasses} ${colorClasses} ${className}`}
...
const GrowSpinner = component$<{
  ...
  return (
    <div
      class={`inline-block rounded-full animate-pulse ${sizeClasses} ${colorClasses} ${className}`}
      style={{ animationDuration }}
      role="status"
      aria-label={ariaLabel}
    />
...
const DotsSpinner = component$<{
  ...
  const { sizeClasses, colorClasses, animationDuration, className, ariaLabel, speed } = props;
  ...
  return (
    ...
    >
      {[0, 1, 2, 3, 4].map((i) => (
        <div 
          key={i}
          class="w-1.5 bg-current animate-grow-y rounded-t-sm"
          style={{ 
            height: '100%',
            animationDuration,
