import { component$, useSignal } from '@builder.io/qwik';
import { Tooltip } from '~/components/Core/DataDisplay/Tooltip';
import { Button } from '~/components/Core/Input/Button';

export default component$(() => {
  const isOpen = useSignal(false);
  
  return (
    <div class="flex flex-col gap-8">
      <div>
        <h3 class="text-sm font-medium mb-2">Interactive Tooltips</h3>
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
          These tooltips allow users to interact with content inside them without the tooltip closing.
        </p>
        
        <div class="flex flex-wrap gap-4">
          <Tooltip
            content={
              <div class="p-1">
                <p class="mb-2">Select your preferred theme:</p>
                <div class="flex gap-2">
                  <Button size="sm" variant="outline">Light</Button>
                  <Button size="sm" variant="outline">Dark</Button>
                  <Button size="sm" variant="outline">System</Button>
                </div>
              </div>
            }
            interactive={true}
            placement="bottom"
            maxWidth="250px"
            color="secondary"
          >
            <Button>Theme Settings</Button>
          </Tooltip>
          
          <Tooltip
            content={
              <div class="p-1">
                <p class="font-medium mb-2">Quick Actions</p>
                <ul class="space-y-1">
                  <li>
                    <button class="w-full text-left hover:bg-gray-100 dark:hover:bg-gray-700 px-2 py-1 rounded">
                      Edit Profile
                    </button>
                  </li>
                  <li>
                    <button class="w-full text-left hover:bg-gray-100 dark:hover:bg-gray-700 px-2 py-1 rounded">
                      Account Settings
                    </button>
                  </li>
                  <li>
                    <button class="w-full text-left hover:bg-gray-100 dark:hover:bg-gray-700 px-2 py-1 rounded">
                      Sign Out
                    </button>
                  </li>
                </ul>
              </div>
            }
            interactive={true}
            placement="right"
            maxWidth="200px"
          >
            <Button variant="outline">Menu</Button>
          </Tooltip>
        </div>
      </div>
      
      <div>
        <h3 class="text-sm font-medium mb-2">Controlled Tooltip</h3>
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
          This tooltip's visibility is controlled programmatically.
        </p>
        
        <div class="flex items-center gap-4">
          <Tooltip
            content="This tooltip is controlled programmatically"
            placement="top"
            color="primary"
            visible={isOpen.value}
            onVisibleChange$={(visible) => isOpen.value = visible}
          >
            <Button>Hover Me</Button>
          </Tooltip>
          
          <Button 
            variant={isOpen.value ? "primary" : "outline"} 
            onClick$={() => isOpen.value = !isOpen.value}
          >
            {isOpen.value ? 'Hide Tooltip' : 'Show Tooltip'}
          </Button>
        </div>
      </div>
    </div>
  );
});
