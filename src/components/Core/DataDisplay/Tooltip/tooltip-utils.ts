...
import type { TooltipColor, TooltipPlacement, TooltipSize } from './Tooltip.types';
...
export const getTooltipColorClasses = (color: TooltipColor | string) => {
  const colorClasses = {
    ...
    primary: 'bg-primary-700 text-white dark:bg-primary-300 dark:text-primary-900',
    secondary: 'bg-secondary-700 text-white dark:bg-secondary-300 dark:text-secondary-900',
...
/**
 * Get arrow transform based on placement
 */
...
export function getTooltipPosition(params: PositionParams): PositionResult {
  const { triggerElement, tooltipElement, arrowElement, placement, offset } = params;
  ...
  const mainPlacement = placementParts[0] as 'top' | 'right' | 'bottom' | 'left';
  const alignment = placementParts[1] as 'start' | 'end' | undefined;
  
  // Get arrow dimensions if it exists
  const arrowWidth = arrowElement ? arrowElement.getBoundingClientRect().width : 0;
  const arrowHeight = arrowElement ? arrowElement.getBoundingClientRect().height : 0;
  ...
  if (alignment) {
    if (['top', 'bottom'].includes(mainPlacement)) {
      if (alignment === 'start') {
        ...
        if (arrowElement) {
          arrowX = Math.min(triggerRect.width / 2 - arrowWidth / 2, tooltipRect.width / 4);
        }
      } else if (alignment === 'end') {
        x = triggerRect.right - tooltipRect.width + scrollX;
        if (arrowElement) {
          arrowX = tooltipRect.width - Math.min(triggerRect.width / 2 + arrowWidth / 2, tooltipRect.width * 3 / 4);
        }
      }
      ...
      if (alignment === 'start') {
        ...
        if (arrowElement) {
          arrowY = Math.min(triggerRect.height / 2 - arrowHeight / 2, tooltipRect.height / 4);
        }
      } else if (alignment === 'end') {
        y = triggerRect.bottom - tooltipRect.height + scrollY;
        if (arrowElement) {
          arrowY = tooltipRect.height - Math.min(triggerRect.height / 2 + arrowHeight / 2, tooltipRect.height * 3 / 4);
        }
      }
    }
  }
