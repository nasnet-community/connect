...
export function useGridItem(props: GridItemProps) {
    ...
    if (typeof value === "object" && value !== null && !Array.isArray(value)) {
      ...
      breakpoints.forEach((bp) => {
        if (responsiveValue[bp] !== undefined) {
          classes[`${bp}:${classPrefix}-${transform(responsiveValue[bp])}`] =
            true;
        }
      });
  ...
  const colSpanClasses = generateResponsiveClasses(
    colSpan,
    "col-span",
    (val) => (val === "full" ? "full" : String(val)),
  );

  // Generate row span classes
  const rowSpanClasses = generateResponsiveClasses(
    rowSpan,
    "row-span",
    (val) => (val === "full" ? "full" : String(val)),
  );
  ...
  const colStartClasses = generateResponsiveClasses(
    colStart,
    "col-start",
    (val) => (val === "auto" ? "auto" : String(val)),
  );
  ...
  const rowStartClasses = generateResponsiveClasses(
    rowStart,
    "row-start",
    (val) => (val === "auto" ? "auto" : String(val)),
  );
  ...
  const justifySelfClasses = generateResponsiveClasses(
    justifySelf,
    "justify-self",
  );

  // Generate align self classes
  const alignSelfClasses = generateResponsiveClasses(
    alignSelf,
    "self",
    (val) => {
