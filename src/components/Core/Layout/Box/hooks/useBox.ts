...
export function useBox(props: BoxProps) {
  ...
  const generatePaddingClasses = (
    ...
    if (paddingObj.x) {
      ...
      Object.assign(
        classes,
        Object.fromEntries(
          Object.entries(xClasses).map(([key, val]) => [
            key.replace("p-", "px-"),
            val,
          ]),
        ),
    ...
    if (paddingObj.y) {
      ...
      Object.assign(
        classes,
        Object.fromEntries(
          Object.entries(yClasses).map(([key, val]) => [
            key.replace("p-", "py-"),
            val,
          ]),
        ),
  ...
  const generateMarginClasses = (
    margin: BoxMargin | object,
  ): Record<string, boolean> => {
  ...
  const paddingClasses = generateResponsiveClasses(
    padding,
    generatePaddingClasses,
  );

  // Generate margin classes with responsive support
  const marginClasses = generateResponsiveClasses(
    margin,
    generateMarginClasses,
  );

  // Enhanced border radius with responsive support
  const borderRadiusClasses = generateResponsiveClasses(
    borderRadius,
    (radius) => ({
  ...
  const borderWidthClasses = generateResponsiveClasses(
    borderWidth,
    (width) => ({
      "border-0": width === "none",
      border: width === "thin",
  ...
  const borderStyleClasses = generateResponsiveClasses(
    borderStyle,
    (style) => ({
  ...
  const borderColorClasses = generateResponsiveClasses(
    borderColor,
    (color) => ({
  ...
  const backgroundColorClasses = generateResponsiveClasses(
    backgroundColor,
    (bg) => ({
      ...
      "bg-surface-light-elevated dark:bg-surface-dark-elevated":
        bg === "surface-elevated",
      "bg-surface-light-depressed dark:bg-surface-dark-depressed":
        bg === "surface-depressed",
  ...
  const focusStyleClasses = {
    "focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 dark:focus:ring-primary-400":
      focusStyle === "ring",
    "focus:outline-2 focus:outline-primary-500 dark:focus:outline-primary-400":
      focusStyle === "outline",
    "focus:shadow-lg focus:shadow-primary-500/25 dark:focus:shadow-primary-400/25":
      focusStyle === "glow",
  ...
  if (viewportWidth) {
    if (
      ...
      ["vh", "vw", "dvh", "svh", "lvh"].includes(viewportWidth)
    ) {
  ...
  if (viewportHeight) {
    if (
      typeof viewportHeight === "string" &&
      ["vh", "vw", "dvh", "svh", "lvh"].includes(viewportHeight)
    ) {
