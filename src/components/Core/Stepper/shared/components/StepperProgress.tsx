...
export const StepperProgress = component$<StepperProgressProps>((props) => {
  ...
  const getConnectorClass = (index: number) => {
    ...
    const positionClass =
      layout === "vertical" ? "" : "left-1/2 -translate-x-1/2";
    ...
    <div class={getProgressClass()} role="navigation" aria-label="Progress">
      {steps.map((step, index) => {
        ...
        const isClickable = (() => {
          ...
          if (index === activeStep + 1 && steps[activeStep]?.isComplete)
            return true;
          return false;
        })();
        ...
        return (
          <div
            key={step.id}
            class={layout === "vertical" ? "relative" : "relative flex-1"}
          >
            <div
            ...
            >
              ...
              {/* Step Label */}
              <div
                class={layout === "vertical" ? "flex-1" : "mt-2 text-center"}
              >
                <p
                ...
                {(step.isOptional || step.skippable) && (
                  <div class="mt-1 flex gap-1">
                    {step.isOptional && (
                      <span class="inline-flex items-center rounded bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">
                        Optional
                      </span>
                    )}
                    {step.skippable && (
                      <span class="inline-flex items-center rounded bg-yellow-100 px-2 py-0.5 text-xs font-medium text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400">
                        Skippable
                      </span>
                    )}
                  </div>
      ...
      {layout === "horizontal" && (
        <div
          class="absolute left-0 right-0 top-5 h-0.5 bg-gray-300 dark:bg-gray-700"
          aria-hidden="true"
        >
          <div
            class="h-full bg-green-600 transition-all duration-300 dark:bg-green-500"
            style={{
