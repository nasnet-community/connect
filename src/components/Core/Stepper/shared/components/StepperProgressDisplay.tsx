...
export const StepperProgressDisplay = component$((props: StepperProgressDisplayProps) => {
  const {
    steps,
    activeStep,
    onStepClick$,
  ...
  const renderStepIndicator = (
    ...
    if (hasErrors) {
      return <span class="text-sm font-semibold text-red-600 dark:text-red-400">!</span>;
    }
    ...
    return (
      <div class="relative space-y-0">
        {steps.map((step, stepIndex) => {
          ...
          const hasErrors = Boolean(step.validationErrors && step.validationErrors.length > 0);
          const canInteract = getCanInteract(stepIndex);
          const sidePosition = verticalSide === "left" ? "1.125rem" : "1.125rem";

          return (
            <div key={`step-${step.id}-vertical-${stepIndex}`} class={`relative pb-4 ${stepIndex === steps.length - 1 ? "pb-0" : ""}`}>
              {stepIndex < steps.length - 1 && (
                <div
                  class={`absolute top-10 bottom-0 w-1 rounded-full ${
                    stepIndex < activeStep ? "bg-yellow-500 dark:bg-yellow-400" : "bg-gray-200 dark:bg-gray-700"
                  }`}
              ...
              >
                ...
                <div class={`min-w-0 flex-1 pt-1 ${step.isDisabled ? "opacity-50" : ""}`}>
                  <div
                  ...
                  {step.isOptional && (
                    <div class="mt-0.5 text-[11px] text-gray-400 dark:text-gray-500">(Optional)</div>
                  )}
                </div>
              </div>
            </div>
  ...
  return (
    <div class="relative w-full" style={{ minHeight: desktopMinHeight }}>
      <div class="absolute left-0 right-0 top-[18px] z-0 h-1">
        <div class="h-1 w-full bg-gray-200 dark:bg-gray-700" aria-hidden="true"></div>
        <div
          class="absolute left-0 top-0 h-1 bg-yellow-500 transition-all duration-500 dark:bg-yellow-400"
          style={{ width: `${steps.length > 1 ? Math.min(100, (activeStep / (steps.length - 1)) * 100) : 0}%` }}
          aria-hidden="true"
        ></div>
      </div>
      ...
      <div
        ...
        style={{ minHeight: desktopMinHeight, minWidth: scrollable ? "min-content" : undefined }}
      >
        {steps.map((step, stepIndex) => {
          const isComplete = activeStep > stepIndex;
          const isCurrent = activeStep === stepIndex;
          ...
          return (
            <div
              ...
              class={`flex flex-col items-center ${scrollable ? "snap-center flex-shrink-0" : ""}`}
            >
              <button
              ...
              <div class={`text-center ${titleMarginClass} ${isCurrent ? "font-medium" : ""}`}>
                <div
                  class={`${titleTextClass} break-words leading-tight hyphens-auto ${
                    isCurrent
                ...
                {step.isOptional && (
                  <div class={`${optionalTextClass} mt-0.5 text-gray-400 dark:text-gray-500`}>(Optional)</div>
                )}
              </div>
            </div>
          );
