 ...
 */
export function createIsolatedStepperContext<
  T = any,
...
>(
  ...
  const contextId = createContextId<BaseStepperContext<T, S>>(
    `stepper-${contextPath}`,
  );
 ...
 */
export interface StepperContextProviderConfig<
  T = any,
  S extends BaseStepMeta = BaseStepMeta,
> {
  contextId: ContextId<BaseStepperContext<T, S>>;
 ...
 */
export function useProvideStepperContext<
  T = any,
  S extends BaseStepMeta = BaseStepMeta,
>(config: StepperContextProviderConfig<T, S>): BaseStepperContext<T, S> {
  const {
    contextId,
    steps,
    activeStep,
  ...
  const internalLastCompletedStep =
    lastCompletedStep || defaultLastCompletedStep;
...
export function useStepperContext<
  T = any,
  S extends BaseStepMeta = BaseStepMeta,
>(contextId: ContextId<BaseStepperContext<T, S>>): BaseStepperContext<T, S> {
