...
export const CStepperContextId =
  createContextId<CStepperContext>("CStepper-context");
...
export function useProvideStepperContext<T = any>(
  ...
  const nextStep$ = $(() => {
    ...
    if (
      currentStep < stepsSignal.value.length - 1 &&
      stepsSignal.value[currentStep].isComplete
    ) {
  ...
  const contextValue: CStepperContext<T> = {
    ...
    removeStep$: $((stepId: number) => {
      const stepIndex = stepsSignal.value.findIndex(
        (step) => step.id === stepId,
      );
...
export function useStepperContext<T = any>(
  contextId: ContextId<
    CStepperContext<T>
  > = CStepperContextId as unknown as ContextId<CStepperContext<T>>,
) {
