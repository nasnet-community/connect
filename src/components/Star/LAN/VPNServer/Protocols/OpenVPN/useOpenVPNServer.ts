...
import type { OpenVpnServerConfig, OvpnAuthMethod, OvpnCipher } from "../../../../StarContext/Utils/VPNServerType";
...
export const useOpenVPNServer = () => {
  ...
  const updateName$ = $((value: string) => updateAdvancedForm$({ name: value }));
  const updateCertificate$ = $((value: string) => updateAdvancedForm$({ certificate: value }));
  const updatePort$ = $((value: number) => updateAdvancedForm$({ port: value }));
  const updateProtocol$ = $((value: NetworkProtocol) => updateAdvancedForm$({ protocol: value }));
  const updateMode$ = $((value: LayerMode) => updateAdvancedForm$({ mode: value }));
  const updateAddressPool$ = $((value: string) => updateAdvancedForm$({ addressPool: value }));
  const updateRequireClientCertificate$ = $((value: boolean) => updateAdvancedForm$({ requireClientCertificate: value }));
  const updateAuth$ = $((value: OvpnAuthMethod) => updateAdvancedForm$({ auth: value }));
  const updateCipher$ = $((value: OvpnCipher) => updateAdvancedForm$({ cipher: value }));
  const updateCertificateKeyPassphrase$ = $((value: string) => updateAdvancedForm$({ certificateKeyPassphrase: value }));
  const updateDefaultProfile$ = $((value: string) => updateAdvancedForm$({ defaultProfile: value }));
  const updateTlsVersion$ = $((value: "any" | "only-1.2") => updateAdvancedForm$({ tlsVersion: value }));
