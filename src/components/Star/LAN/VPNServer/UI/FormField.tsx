...
export const FormField = component$<FormFieldProps>(
  ({
    ...
    return (
      <div class={`mb-4 w-full ${className}`}>
        <label
          for={fieldId}
          class={`text-text-secondary dark:text-text-dark-secondary mb-1.5 block font-medium ${labelSizeClasses[size]}`}
        >
          {label}
          {required && (
            <span class="ml-1 text-error-600 dark:text-error-500">*</span>
          )}
        </label>
        <div class="w-full">
          <Slot />
        ...
        {(errorMessage || helperText) && (
          <p
            class={`mt-1.5 ${size === "sm" ? "text-xs" : "text-sm"} ${messageClasses}`}
          >
            {errorMessage || helperText}
          </p>
        )}
  ...
  ({
    ...
    return (
      <button
        onClick$={handleClick}
        disabled={disabled}
        class={`rounded-lg px-4 py-2 shadow-sm transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 ${buttonClass} ${disabled ? "cursor-not-allowed opacity-50" : ""} ${className}`}
      >
        <Slot />
      </button>
    );
