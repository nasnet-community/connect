...
export const TabNavigation = component$<TabNavigationProps>(
  ({ tabs, activeTab, class: className = "", onSelect$ }) => {
    return (
      <div
        class={`mb-6 border-b border-border dark:border-border-dark ${className}`}
      >
          ...
          {tabs.map((tab) => {
            ...
            return (
              <li key={tab.id} class="mr-2">
                <button
                  class={`inline-block rounded-t-lg border-b-2 p-4 ${activeClasses}`}
