...
export interface InputFieldProps {
  label: string;
  value: string;
  onInput$?: QRL<(event: Event, element: HTMLInputElement | HTMLTextAreaElement) => void>;
  type?: string;
  required?: boolean;
  placeholder?: string;
  helperText?: string;
  ...
  return (
    <div class={`w-full ${className}`}>
      ...
      {helperText && !errorMessage && (
        <p class="mt-1 text-xs text-text-muted dark:text-text-dark-muted">{helperText}</p>
      )}
      
      {errorMessage && (
        <p class="mt-1 text-xs text-error-600 dark:text-error-500">{errorMessage}</p>
      )}
    </div>
  );
}); 