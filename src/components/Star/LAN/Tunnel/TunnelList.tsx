...
export const TunnelList = component$<TunnelListProps>(({ tunnels, expandedSections, toggleSection$ }) => {
  return (
    <div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
      <div class="flex cursor-pointer items-center justify-between p-6" onClick$={() => toggleSection$('tunnelList')}>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-900">
                {tunnels.map((tunnel, index) => (
                  <tr key={index} class="hover:bg-gray-50 dark:hover:bg-gray-800">
                    ...
                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                      {tunnel.type === 'eoip' && `Tunnel ID: ${(tunnel as any).tunnelId}`}
                      {tunnel.type === 'vxlan' && `VNI: ${(tunnel as any).vni}`}
                      {tunnel.type === 'ipip' && (tunnel as any).ipsecSecret && `IPsec Secret: ${(tunnel as any).ipsecSecret}`}
                      {tunnel.type === 'gre' && (tunnel as any).ipsecSecret && `IPsec Secret: ${(tunnel as any).ipsecSecret}`}
                    </td>
                  </tr>
                ))}
              </tbody>
