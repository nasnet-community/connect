...
export const useLAN = ({
  ...
  useTask$(async ({ track }) => {
    ...
    const hasInterfaces =
      starContext.state.LAN.Interface &&
    ...
    if (isEasyMode) {
      ...
      const isDomesticLink =
        wanLinkType === "domestic" || wanLinkType === "both";
      const hasForeignLink =
        wanLinkType === "foreign" || wanLinkType === "both";
      ...
      const hasValidBaseNetworks =
        starContext.state.LAN.Subnets?.BaseSubnets &&
      ...
      const hasValidVPNServerNetworks = (() => {
        const vpnServerSubnets =
          starContext.state.LAN.Subnets?.VPNServerSubnets;
        if (!vpnServerSubnets) return false;

        // Check arrays (Wireguard, OpenVPN)
        const arraySubnets = [
          vpnServerSubnets.Wireguard,
          vpnServerSubnets.OpenVPN,
        ].filter(Boolean);
        for (const subnets of arraySubnets) {
          if (
            Array.isArray(subnets) &&
            !subnets.every((s: any) => s?.subnet && s.subnet.trim() !== "")
          ) {
            return false;
          }
        }
      ...
      if (!hasValidVPNServerNetworks && vpnServers) {
        ...
        if (vpnServers.WireguardServers?.length) {
          defaultSubnets.VPNServerSubnets.Wireguard =
            vpnServers.WireguardServers.map((server: any, index: number) => ({
        ...
        if (vpnServers.OpenVpnServer?.length) {
          defaultSubnets.VPNServerSubnets.OpenVPN =
            vpnServers.OpenVpnServer.map((server: any, index: number) => ({
      ...
      const hasValidDomesticNetworks = starContext.state.LAN.Subnets
        ?.DomesticSubnets
      ...
      if (
        !hasValidDomesticNetworks &&
        domesticLinks &&
        domesticLinks.length >= 1
      ) {
      ...
      const hasValidForeignNetworks = starContext.state.LAN.Subnets
        ?.ForeignSubnets
      ...
      if (
        !hasValidForeignNetworks &&
        foreignLinks &&
        foreignLinks.length >= 1
      ) {
      ...
      const hasValidVPNClientNetworks = (() => {
        const vpnClientSubnets =
          starContext.state.LAN.Subnets?.VPNClientSubnets;
      ...
      if (!hasValidVPNClientNetworks && vpnClientConfigs.length >= 1) {
        ...
        // Group by type
        const wireguardClients = vpnClientConfigs.filter(
          (c) => c.type === "Wireguard",
        );
        const openVpnClients = vpnClientConfigs.filter(
          (c) => c.type === "OpenVPN",
        );
        ...
        if (wireguardClients.length > 0) {
          defaultSubnets.VPNClientSubnets.Wireguard = wireguardClients.map(
            (client) => ({
              name: client.name,
        ...
        if (openVpnClients.length > 0) {
          defaultSubnets.VPNClientSubnets.OpenVPN = openVpnClients.map(
            (client) => ({
              name: client.name,
              subnet: `192.168.${subnetIndex++}.0/24`,
            }),
        ...
        if (ikev2Clients.length > 0) {
          defaultSubnets.VPNClientSubnets.IKev2 = ikev2Clients.map(
            (client) => ({
              name: client.name,
              subnet: `192.168.${subnetIndex++}.0/24`,
            }),
      ...
      if (!hasInterfaces) {
        ...
        if (masterRouter) {
          ...
          if (targetNetwork && availableInterfaces.length > 0) {
            const interfaceConfigs: EthernetInterfaceConfig[] =
