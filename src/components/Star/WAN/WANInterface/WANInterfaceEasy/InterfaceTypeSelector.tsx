...
export const InterfaceTypeSelector = component$<InterfaceTypeSelectorProps>(
  ({ selectedType, onSelect$, availableInterfaces }) => {
    ...
    const isEthernetAvailable =
      ...
      availableInterfaces.Interfaces.ethernet.length > 0;
    const isWirelessAvailable =
      availableInterfaces.Interfaces.wireless &&
      availableInterfaces.Interfaces.wireless.length > 0;
    const isSFPAvailable =
      availableInterfaces.Interfaces.sfp &&
      availableInterfaces.Interfaces.sfp.length > 0;
    const isLTEAvailable =
      availableInterfaces.Interfaces.lte &&
      availableInterfaces.Interfaces.lte.length > 0;
          ...
          {interfaceTypes.map(({ type, isAvailable }) => (
            ...
            >
              <div class="relative z-10 flex flex-col items-center gap-2">
                ...
                <span
                  class={`text-xs font-medium ${!isAvailable ? "text-gray-400 dark:text-gray-500" : selectedType === type ? "text-primary-700 dark:text-primary-300" : "text-gray-700 dark:text-gray-300"}`}
                >
                  {type}
                </span>
                {!isAvailable && (
