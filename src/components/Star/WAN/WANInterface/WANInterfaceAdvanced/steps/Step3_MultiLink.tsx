...
export const Step3_MultiLink = component$<Step3Props>(
  ({ wizardState, wizardActions }) => {
    ...
    const strategy = useSignal<
      "LoadBalance" | "Failover" | "RoundRobin" | "Both"
    >(currentStrategy);
    ...
    const handleStrategyChange = $((value: string) => {
      ...
      if (newStrategy === "LoadBalance" || newStrategy === "Both") {
        strategyConfig.loadBalanceMethod =
      ...
      if (newStrategy === "Failover" || newStrategy === "Both") {
        strategyConfig.failoverCheckInterval =
          wizardState.multiLinkStrategy?.failoverCheckInterval || 10;
        strategyConfig.failoverTimeout =
          wizardState.multiLinkStrategy?.failoverTimeout || 30;
      }

      // Add round robin settings only for RoundRobin strategy
      if (newStrategy === "RoundRobin") {
        strategyConfig.roundRobinInterval =
          wizardState.multiLinkStrategy?.roundRobinInterval || 60;
        strategyConfig.packetMode =
          wizardState.multiLinkStrategy?.packetMode || "connection";
      }
    ...
    const handleWeightChange = $((linkId: string, newWeight: number) => {
      ...
      const updates: Array<{
        id: string;
        updates: Partial<typeof targetLink>;
      }> = [];
      ...
      if (otherLinks.length > 0) {
        const totalOtherWeight = otherLinks.reduce(
        ...
        if (totalOtherWeight > 0) {
          ...
          otherLinks.forEach((link, index) => {
            ...
            const newLinkWeight = Math.max(
              1,
              (link.weight || 0) - weightChange,
            );
    ...
    const handleDragEnd = $(() => {
      if (
        ...
        draggedItem.value !== draggedOverItem.value
      ) {
        const draggedLink = wizardState.links.find(
          (l) => l.id === draggedItem.value,
        );
        const targetLink = wizardState.links.find(
          (l) => l.id === draggedOverItem.value,
        );
        ...
        if (draggedLink && targetLink) {
          ...
          const tempLinks = wizardState.links.map((link) => ({
            ...link,
            priority:
              link.id === draggedItem.value
                ? targetPriority
                : link.id === draggedOverItem.value
                  ? draggedPriority
                  : link.priority || 0,
          }));
    ...
    const linksByPriority = [...wizardState.links].sort(
      (a, b) => (a.priority || 0) - (b.priority || 0),
    );
      ...
      <div class="space-y-6">
            ...
            <div class="mb-6">
              ...
              <Select
                ...
                onChange$={(value: string | string[]) => {
                  ...
                  wizardActions.updateMultiLink({
                    loadBalanceMethod: selectedValue as
                      | "PCC"
                      | "NTH"
                      | "ECMP"
                      | "Bonding",
              ...
              />
              <FormHelperText>
                Choose how traffic will be distributed across multiple links
              </FormHelperText>
            </div>
              ...
              <button
                onClick$={() => {
                  const equalWeight = Math.floor(
                    100 / wizardState.links.length,
                  );
                  const remainder =
                    100 - equalWeight * wizardState.links.length;
                ...
                class="rounded-lg bg-primary-500 px-3 py-1.5 text-sm text-white transition-colors hover:bg-primary-600"
              >
                Auto Balance
              </button>
            </div>
                    ...
                    <div class="mb-2 flex items-center justify-between">
                      ...
                      <div class="flex items-center gap-2">
                        <input
                          ...
                          class="w-14 rounded border border-gray-300 bg-white px-2 py-1 text-center text-sm text-gray-900 focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20 dark:border-gray-600 dark:bg-gray-800 dark:text-white"
                        />
        ...
        {strategy.value === "RoundRobin" && (
          <div class="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
            ...
            <div class="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                <FormLabel for="rotationInterval">
                ...
                />
                <FormHelperText>
                  How often to switch between WAN links (30-300 seconds)
                </FormHelperText>
              </div>
              ...
              <div>
                ...
                <Select
                  ...
                  onChange$={(value: string | string[]) => {
                    const selectedValue = Array.isArray(value)
                      ...
                      : value;
                    wizardActions.updateMultiLink({
                      packetMode: selectedValue as "connection" | "packet",
                    });
                  }}
                  options={[
                ...
                />
                <FormHelperText>
                  How to distribute traffic during rotation
                </FormHelperText>
              </div>
            </div>

            <div class="rounded-lg border border-green-200 bg-green-50 p-3 dark:border-green-700 dark:bg-green-900/10">
              <div class="flex items-start gap-2">
                <svg
                  class="mt-0.5 h-4 w-4 text-green-600 dark:text-green-400"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                <p class="text-sm text-green-700 dark:text-green-300">
                  Traffic will rotate between all available WAN links at the
                  specified interval, ensuring even distribution of usage
                </p>
              </div>
            </div>
          </div>
        ...
        {(strategy.value === "Failover" || strategy.value === "Both") && (
          <div class="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
            ...
            <div class="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                <FormLabel for="failoverCheckInterval">
                ...
                />
                <FormHelperText>
                  How often to check link health (5-60 seconds)
                </FormHelperText>
              </div>

              <div>
                <FormLabel for="failoverTimeout">
                ...
                />
                <FormHelperText>
                  Time before marking link as failed (10-120 seconds)
                </FormHelperText>
              </div>
            </div>
              ...
              {linksByPriority.map((link, index) => {
                ...
                return (
                  <div
                    ...
                    class={`
                      flex cursor-move items-center gap-3 rounded-lg p-4 transition-all
                      ${
                        isDragging
                          ? "scale-95 opacity-50"
                          : isDraggedOver
                            ? "border-primary-500 bg-primary-50 shadow-lg dark:bg-primary-900/10"
                            : index === 0
                              ? "border border-green-200 bg-white dark:border-green-700 dark:bg-gray-700"
            ...
            <div class="mt-4 rounded-lg border border-blue-200 bg-blue-50 p-3 dark:border-blue-700 dark:bg-blue-900/10">
              <div class="flex items-start gap-2">
                <svg
                ...
                <p class="text-sm text-blue-700 dark:text-blue-300">
                  If the primary link fails, traffic automatically switches to
