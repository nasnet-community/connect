...
export const Step2_Connection = component$<Step2Props>(
  ({ wizardState, wizardActions }) => {
    ...
    const getFilteredLinks = () => {
      ...
      return wizardState.links.filter(
        (link) =>
    ...
    const handleConnectionUpdate = $(async (linkId: string, updates: any) => {
      ...
      if (
        updates.connectionType === "DHCP" ||
        updates.connectionType === "LTE"
      ) {
    ...
    // Get link statistics
    const configuredConnections = wizardState.links.filter(
      (l) => l.connectionType,
    ).length;
    const completedConnections = wizardState.links.filter((l) => {
      ...
      if (l.connectionType === "PPPoE") {
        return (
          l.connectionConfig?.pppoe?.username &&
          l.connectionConfig.pppoe.password
        );
      }
      if (l.connectionType === "Static") {
        return (
          l.connectionConfig?.static?.ipAddress &&
                    ...
                    <div class="max-w-md flex-1">
                      <div class="relative">
                        <input
                          ...
                          class="w-full rounded-full border border-gray-200 bg-white py-2 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-black dark:border-gray-700 dark:bg-gray-800 dark:focus:ring-white"
                        />
                        <svg
                ...
                return (
                  <div
                    ...
                    class={`
                      ...
                      ${
                        status === "complete"
                          ? "border-green-300 bg-white hover:border-green-400 hover:shadow-md dark:border-green-700 dark:bg-gray-800 dark:hover:border-green-600"
                          : status === "error"
                            ? "border-red-300 bg-white hover:border-red-400 hover:shadow-md dark:border-red-700 dark:bg-gray-800 dark:hover:border-red-600"
                            : status === "partial"
                              ? "border-yellow-300 bg-white hover:border-yellow-400 hover:shadow-md dark:border-yellow-700 dark:bg-gray-800 dark:hover:border-yellow-600"
                              : "border-orange-400 bg-white hover:border-orange-500 hover:shadow-md dark:border-orange-500 dark:bg-gray-800 dark:hover:border-orange-400"
                      }
                    `}
                  >
