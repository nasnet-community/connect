...
import {
  getLinkErrors,
  isInterfaceConfigurationComplete,
...
export const getLinkStatus = (
  ...
  if (step === "interface") {
    ...
    if (Boolean(link.interfaceName) || Boolean(link.interfaceType))
      return "partial";
    return "incomplete";
  }
...
export const getLinkStatistics = (
  ...
  const completedConnections = links.filter((l) =>
    isLinkConfigurationComplete(l),
  ).length;
...
export const sortLinksByPriority = (
  links: WANWizardState["links"],
): WANWizardState["links"] => {
...
export const initializeWeights = (
  links: WANWizardState["links"],
): Array<{ id: string; updates: { weight: number } }> => {
...
export const initializePriorities = (
  links: WANWizardState["links"],
): Array<{ id: string; updates: { priority: number } }> => {
  return links.map((link, index) => ({
    id: link.id,
    updates: {
      priority: index + 1,
