...
export const StepPriorities = component$<StepPrioritiesProps>(
  ({ wizardState, wizardActions }) => {
    ...
    // Strategy state
    const strategy = useSignal<MultiVPNStrategy>(
      wizardState.multiVPNStrategy?.strategy || "Failover",
    );
    ...
    useTask$(({ cleanup }) => {
      ...
      console.log(
        "[StepPriorities] Current strategy:",
      ...
      if (
        ...
        if (needsWeightInit) {
          console.log(
            "[StepPriorities] Initializing weights for",
            strategy.value,
          );
    ...
    const handleDragEnd = $(async () => {
      if (
        ...
        const draggedVPN = wizardState.vpnConfigs.find(
          (vpn) => vpn.id === draggedItem.value,
        );
        const targetVPN = wizardState.vpnConfigs.find(
          (vpn) => vpn.id === draggedOverItem.value,
        );
        ...
        if (draggedVPN && targetVPN) {
          ...
          const initialUpdates = [
            ...
            {
              id: draggedOverItem.value,
              updates: { priority: draggedPriority },
            },
          ];
          ...
          const tempVPNs = wizardState.vpnConfigs.map((vpn) => ({
            ...vpn,
            priority:
              vpn.id === draggedItem.value
                ? targetPriority
                : vpn.id === draggedOverItem.value
                  ? draggedPriority
                  : vpn.priority || 0,
          }));

          const sortedVPNs = tempVPNs.sort(
            (a, b) => (a.priority || 0) - (b.priority || 0),
          );
    ...
    const handleStrategyChange = $((newStrategy: MultiVPNStrategy) => {
      console.log(
        `[StepPriorities] Strategy changing from ${strategy.value} to ${newStrategy}`,
      );
      ...
      if (newStrategy === "Failover") {
        ...
        console.log(
          `[StepPriorities] ${newStrategy} strategy selected, needsWeightInit:`,
          needsWeightInit,
        );
        ...
        if (needsWeightInit && !initState.weightsInitialized) {
          ...
          console.log(
            "[StepPriorities] Applying weight updates:",
            weightUpdates,
          );
      ...
      console.log(
        "[StepPriorities] Multi-VPN strategy updated:",
        newMultiVPNStrategy,
      );
    });
        ...
        <div class="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
          ...
          {strategy.value === "Failover" && (
            <div class="rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-700 dark:bg-blue-900/20">
              ...
              <div class="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                  ...
                  <Input
                    ...
                    max={60}
                    value={(
                      wizardState.multiVPNStrategy?.failoverCheckInterval || 10
                    ).toString()}
                    onInput$={(e: Event) => {
                ...
                <div>
                  ...
                  <Input
                    ...
                    max={120}
                    value={(
                      wizardState.multiVPNStrategy?.failoverTimeout || 30
                    ).toString()}
                    onInput$={(e: Event) => {
                      const value = parseInt(
                        (e.target as HTMLInputElement).value,
              ...
              <div class="rounded-lg border border-blue-200 bg-blue-50 p-3 dark:border-blue-700 dark:bg-blue-900/10">
            ...
            <div class="rounded-lg border border-green-200 bg-green-50 p-4 dark:border-green-700 dark:bg-green-900/20">
              ...
              <div>
                ...
                <Input
                  ...
                  max={300}
                  value={(
                    wizardState.multiVPNStrategy?.roundRobinInterval || 60
                  ).toString()}
                  onInput$={(e: Event) => {
                    const value = parseInt(
                      (e.target as HTMLInputElement).value,
          ...
          {strategy.value === "LoadBalance" && (
            <div class="rounded-lg border border-orange-200 bg-orange-50 p-4 dark:border-orange-700 dark:bg-orange-900/20">
              ...
              <div class="mb-6">
                ...
                <select
                  value={
                    wizardState.multiVPNStrategy?.loadBalanceMethod || "PCC"
                  }
                  onChange$={(e: Event) => {
                ...
                <button
                  onClick$={() => {
                    const equalWeight = Math.floor(
                      100 / wizardState.vpnConfigs.length,
                    );
                    const remainder =
                      100 - equalWeight * wizardState.vpnConfigs.length;
                    const updates = wizardState.vpnConfigs.map(
                      (vpn, index) => ({
                        id: vpn.id,
                        updates: {
              ...
              <div class="space-y-3">
                {wizardState.vpnConfigs.map((vpn) => {
                  ...
                  return (
                    <div
                      key={vpn.id}
                      class="rounded-lg bg-white p-3 dark:bg-gray-700"
                    >
                          ...
                          <input
                            ...
                            onInput$={(e) => {
                              ...
                              if (!isNaN(value) && value >= 1 && value <= 99) {
                                ...
                                const otherVPNs = wizardState.vpnConfigs.filter(
                                  (v) => v.id !== vpn.id,
                                );
                                const totalOtherWeight = otherVPNs.reduce(
                                ...
                                if (totalOtherWeight > 0) {
                                  otherVPNs.forEach((otherVPN) => {
                                    const proportion =
                                      (otherVPN.weight || 0) / totalOtherWeight;
                                ...
                                const newSum = updates.reduce(
                                  (sum, u) => sum + (u.updates.weight || 0),
                                  0,
                                );
                                if (newSum !== 100 && otherVPNs.length > 0) {
                                  ...
                                  const currentWeight =
                                    updates.find((u) => u.id === firstOther.id)
                                      ?.updates.weight ||
                                    firstOther.weight ||
                                    0;
                            ...
                            class="w-12 rounded border border-gray-300 bg-white px-2 py-1 text-center text-xs text-gray-900 focus:border-orange-500 focus:outline-none focus:ring-2 focus:ring-orange-500/20 dark:border-gray-600 dark:bg-gray-800 dark:text-white"
                          />
                            ...
                            if (totalOtherWeight > 0) {
                              otherVPNs.forEach((otherVPN) => {
                                ...
                                const newWeight = Math.max(
                                  1,
                                  Math.round(
                                    (otherVPN.weight || 0) - diff * proportion,
                                  ),
                                );
                                updates.push({
                                  id: otherVPN.id,
                      ...
                      <div class="mb-2 flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                          <input
                            ...
                            class="w-12 rounded border border-gray-300 bg-white px-2 py-1 text-center text-xs text-gray-900 focus:border-purple-500 focus:outline-none focus:ring-2 focus:ring-purple-500/20 dark:border-gray-600 dark:bg-gray-800 dark:text-white"
                          />
                          <span class="text-xs text-gray-700 dark:text-gray-300">
                            %
                          </span>
        ...
        <div class="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
          ...
          <div class="space-y-3">
            {sortedVPNs.map((vpn, index) => {
              ...
              const isDraggedOver =
                draggedOverItem.value === vpn.id &&
                draggedItem.value !== vpn.id;
