...
export interface UseSSTPConfigResult {
  ...
  validateSSTPConfig: QRL<(config: SstpClientConfig) => Promise<{
    isValid: boolean;
    emptyFields: string[];
  }>>;
  updateContextWithConfig$: QRL<(parsedConfig: SstpClientConfig) => Promise<void>>;
...
export const useSSTPConfig = (
  ...
  const parseSSTPConfig = $(
    (configText: string): SstpClientConfig | null => {
      try {
        ...
        for (const line of lines) {
          ...
          if (line.startsWith('server ') || line.startsWith('remote ')) {
            ...
            if (version === "any" || version === "only-1.2" || version === "only-1.3") {
        ...
        if (!config.Server?.Address || !config.Credentials?.Username || !config.Credentials?.Password) {
          return null;
        }
