...
export function getRouterFamily(model: string): RouterFamily {
  ...
  if (
    normalizedModel.startsWith("rb") ||
    normalizedModel.includes("rb5009") ||
    normalizedModel.includes("l009")
  ) {
    return "RB";
  }
...
export function categorizeRouters(routers: RouterData[]): RouterCategory[] {
  const categories: Record<RouterFamily, RouterCategory> = {
    ...
    Chateau: {
      ...
      description:
        "LTE/5G Routers - Advanced connectivity with cellular support",
      features: [
        "LTE/5G Connectivity",
        "Dual-band WiFi",
        "Failover Capability",
  ...
  const priorityOrder: RouterFamily[] = [
    "hAP",
    "Chateau",
    "cAP",
    "RB",
...
export function getTotalRouterCount(categories: RouterCategory[]): number {
  return categories.reduce(
    (total, category) => total + category.routers.length,
    0,
  );
}
