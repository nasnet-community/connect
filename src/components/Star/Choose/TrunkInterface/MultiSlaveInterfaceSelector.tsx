  ...
  (props: MultiSlaveInterfaceSelectorProps) => {
    ...
    const handleMappingUpdate = $(
      (
        ...
        let mapping = newMappings.find(
          (m) => m.slaveRouterIndex === slaveIndex,
        );
        ...
        if (field === "slaveInterface") {
          const nonMasterIndices = starContext.state.Choose.RouterModels.map(
            (rm, i) => (!rm.isMaster ? i : -1),
          ).filter((i) => i !== -1);
        ...
        if (allConfigured) {
          ...
          const finalModels = starContext.state.Choose.RouterModels.map(
            (model, idx) => {
              ...
              if (model.isMaster) {
                ...
                masterInterfaces.forEach((intf) => {
                  console.log(
                    `Adding ${intf} to master router's OccupiedInterfaces`,
                  );
                ...
                const slaveMapping = newMappings.find(
                  (m) => m.slaveRouterIndex === relativeSlaveIndex,
                );
                const interfaceToAdd = slaveMapping?.slaveInterface;

                if (interfaceToAdd) {
                  console.log(
                    `Adding ${interfaceToAdd} to slave router ${relativeSlaveIndex}'s OccupiedInterfaces`,
                  );
                  finalModel.Interfaces.OccupiedInterfaces =
              ...
              if (model.MasterSlaveInterface && !model.isMaster) {
                ...
                const hasInterface = occupied.some(
                  (item) => item.interface === model.MasterSlaveInterface,
                );

                if (!hasInterface) {
                  console.warn(
                    `Warning: Slave router at index ${idx} has MasterSlaveInterface ${model.MasterSlaveInterface} but it's not in OccupiedInterfaces`,
                  );
                }
              }
    ...
    const getSlaveMapping = (slaveIndex: number) => {
      return currentMappings.value.find(
        (m) => m.slaveRouterIndex === slaveIndex,
      );
    };
    ...
    return (
      ...
      >
        {/* Master Router Info */}
        {masterRouter && (
          <div class="mx-auto max-w-4xl rounded-xl bg-surface/50 p-6 dark:bg-surface-dark/50">
            ...
            <div class="bg-surface-secondary/30 dark:bg-surface-dark-secondary/30 rounded-lg p-4">
              ...
              <div class="flex flex-wrap gap-2">
                {[
                  ...
                  const usageCount = getUsedMasterInterfaces().filter(
                    (i) => i === intf,
                  ).length;
                  return (
                    <span
                    ...
                    >
                      ...
                      {intf}
                      {usageCount > 0 &&
                        ` (${usageCount} slave${usageCount > 1 ? "s" : ""})`}
                    </span>
                  );
                })}
              </div>
        ...
        <div class="mx-auto max-w-4xl space-y-6">
          {slaveRouters.map((slaveRouter, index) => {
            ...
            return (
              <div
                key={`slave-${index}`}
                class="overflow-hidden rounded-xl bg-surface/50 dark:bg-surface-dark/50"
              >
                {/* Slave Router Header */}
                ...
                <div class="space-y-4 p-4">
                      ...
                      >
                        ...
                        {masterRouter &&
                          getAvailableInterfaces(
                            ...
                            const isUsed =
                              getUsedMasterInterfaces().includes(intf) &&
                  ...
                  {currentMapping?.masterInterface &&
                    currentMapping.slaveInterface && (
                      <div class="mt-2 rounded-lg bg-info/10 p-3 dark:bg-info/15">
              ...
              currentMappings.value.map((mapping, idx) => (
                <div key={idx} class="flex items-center gap-3 text-sm">
                  ...
                  <span class="text-text-secondary dark:text-text-dark-secondary">
                    {mapping.slaveRouterModel}: {mapping.masterInterface} ↔{" "}
                    {mapping.slaveInterface}
