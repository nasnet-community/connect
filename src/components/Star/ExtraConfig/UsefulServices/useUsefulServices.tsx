...
export const useUsefulServices = (): UseUsefulServicesReturn => {
  ...
  const updateAdvancedData = $((data: Partial<AdvancedServicesData>) => {
    ...
    const hasEnabledService =
      services.certificate?.SelfSigned ||
      services.certificate?.LetsEncrypt ||
      (services.ntp?.servers && services.ntp.servers.length > 0) ||
      services.graphing?.Interface ||
      services.graphing?.Queue ||
      services.graphing?.Resources ||
      (services.cloudDDNS?.ddnsEntries &&
        services.cloudDDNS.ddnsEntries.length > 0) ||
    ...
    (services: {
      ...
      const usefulServicesConfig = {
        ...
        certificate: services.certificate
          ? { SelfSigned: true, LetsEncrypt: false }
          : { SelfSigned: false, LetsEncrypt: false },
        ntp: services.ntp
          ? { servers: ["pool.ntp.org"], updateInterval: "1h" as const }
          : { servers: [], updateInterval: "1h" as const },
        graphing: services.graphing
          ? { Interface: true, Queue: true, Resources: true }
          : { Interface: false, Queue: false, Resources: false },
