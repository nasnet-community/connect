...
export const CertificateStep = component$(() => {
  ...
  const enableSelfSigned = useSignal(
    servicesData.certificate.enableSelfSigned || false,
  );
  const enableLetsEncrypt = useSignal(
    servicesData.certificate.enableLetsEncrypt || false,
  );

  // Certificate passphrase state
  const certificatePassphrase = useSignal(
    starCtx.state.LAN.VPNServer?.CertificatePassphrase || "",
  );
  ...
  const validateAndUpdate$ = $(() => {
    ...
    const hasOpenVPN = vpnServer?.OpenVpnServer?.some(
      (server: any) => server?.enabled,
    );
    ...
    const passphraseValid =
      !enableSelfSigned.value || certificatePassphrase.value.length >= 10;
    const isComplete =
      (!requiresCertificate.value ||
        enableSelfSigned.value ||
        enableLetsEncrypt.value) &&
    ...
    <div class="animate-fade-in-up space-y-8">
      ...
      <div class="space-y-6 text-center">
        <div class="relative inline-flex items-center justify-center">
          {/* Subtle glow effect */}
          <div class="absolute inset-0 h-24 w-24 animate-pulse-slow rounded-3xl bg-gradient-to-br from-primary-500/15 via-warning-500/15 to-primary-500/15"></div>
          <div class="relative inline-flex h-24 w-24 items-center justify-center rounded-3xl bg-gradient-to-br from-primary-500 via-primary-600 to-warning-500 text-white shadow-2xl shadow-primary-500/30 transition-all duration-500 hover:rotate-3 hover:scale-110 hover:shadow-primary-500/40">
            <svg
      ...
      {requiresCertificate.value && (
        <Card class="relative animate-fade-in-up overflow-hidden border border-warning-200 bg-gradient-to-r from-warning-50 to-warning-100 shadow-lg dark:border-warning-700 dark:from-warning-900/30 dark:to-warning-800/30">
          <CardBody>
            <div class="flex items-center gap-4">
              <div class="flex h-12 w-12 animate-pulse-subtle items-center justify-center rounded-xl bg-gradient-to-br from-warning-500/20 to-warning-600/20 text-warning-600 dark:text-warning-400">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-7 w-7"
                  fill="none"
      ...
      <Card class="relative overflow-hidden border border-gray-200 bg-white shadow-xl transition-all duration-300 hover:shadow-2xl dark:border-gray-700 dark:bg-gray-800">
        ...
        <CardBody class="space-y-6">
          <div class="grid grid-cols-1 gap-6">
            ...
            <div class="group relative animate-fade-in-up overflow-hidden rounded-2xl border border-gray-200 bg-white p-6 transition-all duration-300 hover:scale-[1.02] hover:border-primary-300 hover:shadow-lg dark:border-gray-700 dark:bg-gray-800 dark:hover:border-primary-500">
              {/* Gradient overlay */}
              <div class="absolute inset-0 bg-gradient-to-r from-primary-500/0 via-primary-500/0 to-primary-500/0 opacity-0 transition-opacity duration-500 group-hover:opacity-5"></div>
              <div class="relative flex items-center justify-between">
                <div class="flex items-center gap-5">
                  <div class="flex h-14 w-14 items-center justify-center rounded-xl bg-gradient-to-br from-primary-500 to-primary-600 text-white shadow-lg transition-all duration-300 group-hover:rotate-6 group-hover:scale-110">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                ...
                <Toggle
                  ...
                  disabled={
                    requiresCertificate.value && !enableLetsEncrypt.value
                  }
                  onChange$={$((checked) => {
            ...
            <div
              class="group relative animate-fade-in-up overflow-hidden rounded-2xl border border-gray-200 bg-white p-6 transition-all duration-300 hover:scale-[1.02] hover:border-success-300 hover:shadow-lg dark:border-gray-700 dark:bg-gray-800 dark:hover:border-success-500"
              style="animation-delay: 0.2s"
            >
              {/* Gradient overlay */}
              <div class="absolute inset-0 bg-gradient-to-r from-success-500/0 via-success-500/0 to-success-500/0 opacity-0 transition-opacity duration-500 group-hover:opacity-5"></div>
              <div class="relative flex items-center justify-between">
                <div class="flex items-center gap-5">
                  <div class="flex h-14 w-14 items-center justify-center rounded-xl bg-gradient-to-br from-success-500 to-success-600 text-white shadow-lg transition-all duration-300 group-hover:rotate-6 group-hover:scale-110">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      class="h-7 w-7 transition-transform duration-300 group-hover:rotate-12"
                      fill="none"
      ...
      {enableSelfSigned.value && !certificatePassphrase.value && (
        <Card class="relative animate-fade-in-up overflow-hidden border border-gray-200 bg-white shadow-xl transition-all duration-300 hover:shadow-2xl dark:border-gray-700 dark:bg-gray-800">
          <CardHeader>
            <h4 class="flex items-center gap-3 text-2xl font-bold text-gray-900 dark:text-white">
              <svg
                xmlns="http://www.w3.org/2000/svg"
