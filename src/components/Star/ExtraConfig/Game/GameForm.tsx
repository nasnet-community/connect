...
export const GameForm = component$<{ context: StarContextType }>(({ context }) => {
        ...
        <button
          onClick$={() => {
            ...
            const newGame = {
              ...
              link: link.value,
              ports: {
                tcp: tcpPorts.value ? tcpPorts.value.split(",").map(p => p.trim()) : [],
                udp: udpPorts.value ? udpPorts.value.split(",").map(p => p.trim()) : []
              }
            };
