 ...
 */
function calculateSubnetInfo(ip: string, prefix: string | number): SubnetInfo | null {
    ...
    try {
        ...
        const prefixLen = typeof prefix === 'string' ? parseInt(prefix, 10) : prefix;
        ...
        const firstHostAddress = prefixLen === 31 ? networkAddress : intToIP(networkInt + 1);
        const lastHostAddress = prefixLen === 31 ? broadcastAddress : intToIP(broadcastInt - 1);
        
        // Determine network class
        const getNetworkClass = (firstOctet: number): 'A' | 'B' | 'C' | 'D' | 'E' | 'Invalid' => {
        ...
        const toBinary = (num: number): string => 
            num.toString(2).padStart(32, '0').replace(/(.{8})/g, '$1.').slice(0, -1);
        ...
        const previousNetwork = networkInt === 0 ? '0.0.0.0' : intToIP(prevNetworkInt);
        
        // Calculate supernet (if possible)
        const supernet = prefixLen > 0 ? `${intToIP(networkInt & ((-1 << (33 - prefixLen)) >>> 0))}/${prefixLen - 1}` : null;
 ...
 */
function calculateSubnets(networkIP: string, currentPrefix: number, newPrefix: number): SubnetInfo[] {
    if (newPrefix <= currentPrefix) {
        throw new Error('New prefix must be larger than current prefix for subnetting');
    }
 ...
 */
function generateIPRange(networkIP: string, prefix: number, includeNetworkBroadcast: boolean = false): string[] {
...
function subnetMaskToCIDR(subnetMask: string): number | null {
    ...
    const expectedMask = prefixLength === 0 ? 0 : (-1 << (32 - prefixLength)) >>> 0;
 ...
 */
function calculateVLSM(baseNetwork: string, hostRequirements: number[]): SubnetInfo[] {
    ...
    for (const hostCount of sortedRequirements) {
        ...
        if (requiredPrefix < basePrefix) {
            throw new Error(`Required subnet size (/${requiredPrefix}) is larger than base network (/${basePrefix})`);
        }
