 ...
 */
export const extractConnectionMark = (
    command: string,
): { newMark: string; connectionMark: string } => {
    // Match new-connection-mark="value" or new-connection-mark=value
    const newMarkMatch = command.match(
    ...
    const connMarkMatch = command.match(
        /connection-mark=(?:"([^"]+)"|([^\s]+))/,
    );
    const connectionMark = connMarkMatch
        ? connMarkMatch[1] || connMarkMatch[2]
        : "";
 ...
 */
export const getLoadBalancingType = (
    command: string,
    comment: string,
): string => {
 ...
 */
export const getManglePriority = (
    command: string,
    comment: string,
): ManglePriority => {
    ...
    if (
        ...
        lowerComment.includes("routing games")
    ) {
    ...
    if (
        (lowerComment.includes("domestic") ||
...
export const getSubPriority = (
    ...
    if (priority === ManglePriority.VPNEndpoint) {
        ...
        if (
            lowerCommand.includes("mark-routing") &&
            lowerCommand.includes("connection-mark")
        )
            return 1;
        if (
            lowerCommand.includes("mark-routing") &&
    ...
    if (priority === ManglePriority.Split) {
        ...
        if (
            lowerComment.includes("split-dom") &&
            !lowerComment.includes("!dom")
        )
            return 2;
        if (lowerComment.includes("split-!dom")) return 3;
        if (
            lowerCommand.includes("chain=forward") &&
            lowerCommand.includes("mark-connection")
        )
            return 4;
        if (
            lowerCommand.includes("chain=prerouting") &&
            lowerCommand.includes("mark-routing")
        )
            return 5;
        return 999;
    ...
    if (priority === ManglePriority.NetworkRouting) {
        ...
        let networkOrder = 0;
        if (
            target.startsWith("Domestic") ||
            lowerComment.includes("domestic")
        ) {
            networkOrder = 0;
        } else if (
            target.startsWith("Foreign") ||
            lowerComment.includes("foreign")
        ) {
        ...
        if (loadBalancingType === "NTH" || loadBalancingType === "PCC") {
            ...
            const interfaceMatch = comment.match(
                /Mark ([^\s]+) (connections|routing)/,
            );
            ...
            if (loadBalancingType === "NTH") {
                ...
                if (
                    lowerCommand.includes("mark-connection") &&
                    chain === "prerouting" &&
                    lowerCommand.includes("in-interface")
                ) {
 ...
 */
export const sortRouterConfig = (
    config: RouterConfig,
    sections: string[] = ["/ip firewall mangle"],
): RouterConfig => {
