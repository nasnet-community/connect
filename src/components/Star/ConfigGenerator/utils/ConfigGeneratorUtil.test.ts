...
describe('ConfigGeneratorUtil', () => {
  describe('mergeRouterConfigs', () => {
    it('should merge two RouterConfigs correctly', () => {
      ...
      validateRouterConfig(result, ['/ip address', '/system identity', '/ip route']);
  ...
  describe('CommandShortner', () => {
    it('should not modify comments (lines starting with #)', () => {
      ...
      expect(result[''][1]).toBe('# Another comment with many words that exceeds three parts but should not be modified');
      expect(result['/ip address'][0]).toBe('# Comment in section');
       ...
       expect(result['/ip address'][1]).toBe('add address=192.168.1.1/24 interface=ether1');
    ...
    it('should break long commands into multiple lines with continuation characters', () => {
      ...
      expect(command).toContain('connection-state=established,related protocol=tcp dst-port=80,443');
    });
    ...
    it('should break commands containing single quotes when they have more than 4 parts', () => {
      ...
      expect(command).toContain("comment='test script with single quotes'");
    });
    ...
    it('should handle very long commands with proper line breaking', () => {
      ...
      const rejoined = lines.join(' ').replace(/ \\/g, '').replace(/\n\s+/g, ' ');
    ...
    it('should handle config with empty sections', () => {
      ...
      expect(result['/system identity']).toEqual(['set name=Router']);
       ...
       expect(lines[0]).toBe('add action=accept chain=input connection-state=established,related \\');
       
       // Second line should have 3 elements + backslash (indented)
       expect(lines[1]).toBe('    protocol=tcp dst-port=80,443 src-address=192.168.1.0/24 \\');
     ...
     it('should preserve command structure after line breaking', () => {
      ...
      expect(reconstructed).toBe('add name=bridge1 protocol-mode=rstp priority=0x8000 admin-mac=auto auto-mac=yes ageing-time=5m max-message-age=20s');
    });
       ...
       const lines = command.split('\n');
       expect(lines[0]).toBe('add dst-address=0.0.0.0/0 gateway=192.168.1.1 \\');
    ...
    it('should intelligently parse quoted strings as single tokens', () => {
      ...
      expect(command).toBe('add interface="wireguard-server" list="LAN" comment="VPN interface"');
    });
    ...
    it('should break commands with simple quoted strings when they exceed 4 parts', () => {
      ...
      expect(lines[0]).toBe('add address="192.168.1.0/24" list="LAN-Nets" comment="Main LAN" \\');
    ...
    it('should integrate well with SConfigGenerator', () => {
      ...
      console.log('\n' + '='.repeat(80));
      console.log(`🧪 Testing: CommandShortner + SConfigGenerator Integration`);
  ...
  describe('Integration tests', () => {
    it('should work end-to-end: merge configs and generate output', () => {
      ...
      const merged = mergeRouterConfigs(networkConfig, systemConfig, routingConfig);
