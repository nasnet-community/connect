...
describe('IPAddress Utilities', () => {
  ...
  describe('calculateSubnets', () => {
    ...
    it('should throw error for invalid subnetting', () => {
      expect(() => calculateSubnets('192.168.1.0', 26, 24))
        .toThrow('New prefix must be larger than current prefix');
    });
  });
  ...
  describe('calculateVLSM', () => {
    ...
    it('should throw error for oversized requirements', () => {
      ...
      expect(() => calculateVLSM('192.168.1.0/24', hostRequirements))
        .toThrow();
    });
    ...
    it('should throw error for invalid base network', () => {
      expect(() => calculateVLSM('invalid/24', [10]))
        .toThrow('Invalid base network');
    });
  });
  ...
  describe('Binary representations', () => {
    it('should include binary representations in subnet info', () => {
      ...
      expect(result!.binaryIP).toMatch(/^[01]{8}\.[01]{8}\.[01]{8}\.[01]{8}$/);
      expect(result!.binaryMask).toMatch(/^[01]{8}\.[01]{8}\.[01]{8}\.[01]{8}$/);
      expect(result!.binaryNetwork).toMatch(/^[01]{8}\.[01]{8}\.[01]{8}\.[01]{8}$/);
      expect(result!.binaryBroadcast).toMatch(/^[01]{8}\.[01]{8}\.[01]{8}\.[01]{8}$/);
      
      // Verify binary IP is correct for 192.168.1.1
      expect(result!.binaryIP).toBe('11000000.10101000.00000001.00000001');
    });
  });
