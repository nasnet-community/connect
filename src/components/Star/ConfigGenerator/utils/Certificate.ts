...
export const PrivateCert = (
    ...
    const privateCertScriptContent: RouterConfig = {
        "": [
            ...
            `:do {`,
            `    # Get router's primary IP for CRL host instead of hardcoded localhost`,
            `    :local crlHost "127.0.0.1";`,
            `    :local routerIP [/ip address get [find interface~"ether" !invalid] address];`,
            `    :if ([:len $routerIP] > 0) do={`,
            `        :set crlHost [:pick $routerIP 0 [:find $routerIP "/"]];`,
            `    }`,
            ...
            `# Try to find LAN IP - check bridge first, then ether interfaces`,
            `:local lanIP "";`,
            `:local lanIPOnly "";`,
            `:local bridgeIP [/ip address find interface~"bridge" !invalid];`,
            `:if ([:len $bridgeIP] > 0) do={`,
            `    :set lanIP [/ip address get [:pick $bridgeIP 0] address];`,
            `} else={`,
            `    # Fallback to first ether interface IP`,
            `    :local etherIP [/ip address find interface~"ether" !invalid];`,
            `    :if ([:len $etherIP] > 0) do={`,
            `        :set lanIP [/ip address get [:pick $etherIP 0] address];`,
            `    }`,
            `}`,
            `:if ([:len $lanIP] > 0) do={`,
            `    :set lanIPOnly [:pick $lanIP 0 [:find $lanIP "/"]];`,
            `}`,
            "",
            `:do {`,
            `    :local serverCertCN ("Server@". $"CN_CA_NAME");`,
            `    :local sanValue ("DNS:". $"ROUTER_IDENTITY");`,
            `    :if ([:len $lanIPOnly] > 0) do={`,
            `        :set sanValue ($sanValue. ",IP:". $lanIPOnly);`,
            `    }`,
            ...
            `        country=$COUNTRY state=$STATE locality=$LOC organization=$ORG unit=$OU \\`,
            `        subject-alt-name=$sanValue;`,
            `    :log info ("Server certificate template created with SAN: ". $sanValue);`,
            `} on-error={`,
            ...
            `:put "=== Setup Complete ===";`,
            `:put "Note: Certificates created but not applied to services.";`,
            `:put "To apply server certificate to services manually:";`,
...
export const ExportCert = (
    ...
    const exportCertScriptContent: RouterConfig = {
        "": [
            ...
            `:global "ROUTER_IDENTITY" [/system identity get name];`,
            `:global "CN_CA_NAME" ($"ROUTER_IDENTITY" . "-CA");`,
            `:local "CERT_PASSWORD" "${certPassword}";`,
            "",
            `:log info "Starting generic client certificate export...";`,
            "",
            "# --- Step 1: Verify CA Certificate Exists ---",
            `:put "--> Checking for CA certificate '$"CN_CA_NAME"'...";`,
            `:local caCertEntry [/certificate find name=$"CN_CA_NAME" authority=yes];`,
            `:if ([:len $caCertEntry] = 0) do={`,
            `    :log error "CA certificate '$"CN_CA_NAME"' not found. Run PrivateCert setup first.";`,
            `    :put "ERROR: CA certificate '$"CN_CA_NAME"' not found. Aborting.";`,
            `} else={`,
            `    :log info "CA certificate '$"CN_CA_NAME"' found.";`,
            `    :put "CA certificate '$"CN_CA_NAME"' found.";`,
            "",
            `    # --- Step 2: Verify Client Template Exists ---`,
            `    :put "--> Checking for Client-Template certificate...";`,
            `    :local clientTemplateEntry [/certificate find name="Client-Template"];`,
            `    :if ([:len $clientTemplateEntry] = 0) do={`,
            `        :log error "Client-Template certificate not found. Run PrivateCert setup first.";`,
            `        :put "ERROR: Client-Template certificate not found. Aborting.";`,
            `    } else={`,
            `        :log info "Client-Template certificate found.";`,
            `        :put "Client-Template certificate found.";`,
            "",
            `        # --- Step 3: Create and Sign Generic Client Certificate ---`,
            `        :local genericClientName ("Client@". $"CN_CA_NAME");`,
            `        :put "--> Creating generic client certificate...";`,
            "",
            `        # Check if generic client certificate already exists`,
            `        :local existingGenericCert [/certificate find common-name=$genericClientName];`,
            `        :if ([:len $existingGenericCert] > 0) do={`,
            `            :log warning "Generic client certificate '$genericClientName' already exists. Removing old certificate.";`,
            `            /certificate remove $existingGenericCert;`,
            `            :delay 2s;`,
            `        }`,
            "",
            `        # Create generic client certificate from template`,
            `        :do {`,
            `            /certificate add name="Generic-Client-Template" copy-from=Client-Template \\`,
            `                common-name=$genericClientName;`,
            `            :log info "Generic client certificate template created";`,
            `            :put "Generic client certificate template created.";`,
            `        } on-error={`,
            `            :log error "Failed to create generic client certificate template";`,
            `            :put "ERROR: Failed to create generic client certificate template.";`,
            `        }`,
            "",
            `        # Sign the generic client certificate`,
            `        :put "--> Signing generic client certificate...";`,
            `        :do {`,
            `            /certificate sign "Generic-Client-Template" ca=$"CN_CA_NAME" \\`,
            `                name=$genericClientName;`,
            `            :delay 5s;`,
            `            :log info "Generic client certificate '$genericClientName' created and signed.";`,
            `            :put "Generic client certificate '$genericClientName' created and signed.";`,
            `        } on-error={`,
            `            :log error "Failed to sign generic client certificate";`,
            `            :put "ERROR: Failed to sign generic client certificate.";`,
            `        }`,
            "",
            `        # --- Step 4: Export Certificates ---`,
            `        :put "--> Exporting CA certificate...";`,
            `        :do {`,
            `            /certificate export-certificate $"CN_CA_NAME" export-passphrase="" \\`,
            `                file-name="ca_certificate";`,
            `            :log info "CA certificate exported as 'ca_certificate.crt'";`,
            `            :put "CA certificate exported as 'ca_certificate.crt'";`,
            `        } on-error={`,
            `            :log error "Failed to export CA certificate";`,
            `            :put "ERROR: Failed to export CA certificate.";`,
            `        }`,
            "",
            `        :put "--> Exporting generic client certificate with passphrase...";`,
            `        :do {`,
            `            /certificate export-certificate $genericClientName \\`,
            `                export-passphrase=$"CERT_PASSWORD" \\`,
            `                file-name="client_certificate";`,
            `            :log info "Client certificate exported as 'client_certificate.crt' and 'client_certificate.key'";`,
            `            :put "Client certificate exported as 'client_certificate.crt' and 'client_certificate.key'";`,
            `            :put "Private key is protected with passphrase: '$CERT_PASSWORD'";`,
            `        } on-error={`,
            `            :log error "Failed to export client certificate";`,
            `            :put "ERROR: Failed to export client certificate.";`,
            `        }`,
            "",
            `        # --- Step 5: Export PEM Bundle (for broad compatibility) ---`,
            `        :put "--> Exporting PEM bundle for broad client compatibility...";`,
            `        :do {`,
            `            /certificate export-certificate $genericClientName \\`,
            `                export-passphrase=$"CERT_PASSWORD" \\`,
            `                file-name="client_bundle" \\`,
            `                type=pem;`,
            `            :log info "PEM bundle exported as 'client_bundle.crt' and 'client_bundle.key'";`,
            `            :put "PEM bundle exported as 'client_bundle.crt' and 'client_bundle.key'";`,
            `            :put "PEM private key passphrase: '$CERT_PASSWORD'";`,
            `        } on-error={`,
            `            :log error "Failed to export PEM bundle";`,
            `            :put "WARNING: Failed to export PEM bundle.";`,
            `        }`,
            "",
            `        # --- Summary ---`,
            `        :log info "Generic client certificate export completed";`,
            `        :put "=== Generic Client Certificate Export Summary ===";`,
            `        :put "Certificate CN: $genericClientName";`,
            `        :put "Files created in /file:";`,
            `        :put "  - ca_certificate.crt (CA certificate)";`,
            `        :put "  - client_certificate.crt (Client certificate)";`,
            `        :put "  - client_certificate.key (Client private key)";`,
            `        :put "  - client_bundle.crt (PEM certificate)";`,
            `        :put "  - client_bundle.key (PEM private key)";`,
            `        :put "Passphrase for private keys: '$CERT_PASSWORD'";`,
            `        :put "";`,
            `        :put "Usage: These certificates can be used by any VPN client.";`,
            `        :put "The same certificate files can be distributed to multiple users.";`,
            `        :put "=== Export Complete ===";`,
            `    }`,
            `}`,
            "",
