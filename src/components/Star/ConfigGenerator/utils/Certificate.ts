...
export const AllCert = (config: AllCertConfig = {}): RouterConfig => {
    ...
    const exportOpenVPN = ExportOpenVPN();
...
export const DiagnosticLetsEncryptAdvanced = (): RouterConfig => {
    ...
    const advancedDiagnosticContent: RouterConfig = {
        "": [
            ...
            `        }`,
            `    } else={`,
            `        :log info "ℹ WWW Service: Currently disabled (will be enabled during cert request)";`,
            `    }`,
            `    :if ($wwwPort != 80) do={`,
            `        :log error ("✗ WWW Service: Port is " . $wwwPort . " instead of 80");`,
            ...
            `    }`,
            `} else={`,
            `    :log info "ℹ Firewall: No specific port 80 rules (will add temporary during cert request)";`,
            `}`,
            "",
...
export const ExportOpenVPN = (): RouterConfig => {
    ...
    const exportOpenVPNScriptContent: RouterConfig = {
        "": [
            ...
            "# Version 3.0 - Enhanced Error Handling and Logging",
            "# Exports .ovpn configuration files for OpenVPN clients",
            "",
            "# Enhanced logging and error handling configuration",
            `:log info "=== OPENVPN CLIENT CONFIGURATION EXPORT STARTED ===";`,
            `:log info "Script Version: 3.0 - Enhanced Error Handling and Logging";`,
            `:local startTime [/system clock get time];`,
            `:local startDate [/system clock get date];`,
            `:log info ("Execution started at: " . $startDate . " " . $startTime);`,
            `:log info ("Router Identity: " . [/system identity get name]);`,
            ...
            `:local logOperation do={`,
            `    :local level $1;`,
            `    :local message $2;`,
            `    :local timestamp ("[" . [/system clock get time] . "] ");`,
            `    :local logEntry ($timestamp . $level . ": " . $message);`,
            `    :set ($operationLog->[:len $operationLog]) $logEntry;`,
            `    :log ($level) $logEntry;`,
            `    :put $logEntry;`,
            `}`,
            "",
            "# Helper function to handle errors with categorization",
            `:local handleError do={`,
            `    :local errorType $1;`,
            `    :local errorMessage $2;`,
            `    :local isCritical $3;`,
            `    :local timestamp ("[" . [/system clock get time] . "] ");`,
            `    :local fullMessage ($timestamp . $errorType . ": " . $errorMessage);`,
            `    `,
            `    :if ($isCritical) do={`,
            `        :set ($criticalErrors->[:len $criticalErrors]) $fullMessage;`,
            `        :set errorCount ($errorCount + 1);`,
            `        :log error $fullMessage;`,
            `        :put ("CRITICAL ERROR: " . $fullMessage);`,
            `    } else={`,
            `        :set ($warnings->[:len $warnings]) $fullMessage;`,
            `        :set warningCount ($warningCount + 1);`,
            `        :log warning $fullMessage;`,
            `        :put ("WARNING: " . $fullMessage);`,
            `    }`,
            `}`,
            "",
            "# Configuration Variables with validation",
            ...
            `:do {`,
            `    :set $"ROUTER_IDENTITY" [/system identity get name];`,
            `    :if ([:len $"ROUTER_IDENTITY"] = 0) do={`,
            `        $handleError "IDENTITY_ERROR" "Router identity is empty or not set" true;`,
            `        :error "Cannot proceed without router identity";`,
            `    } else={`,
            `        $logOperation "info" ("Router Identity validated: " . $"ROUTER_IDENTITY");`,
            `    }`,
            `} on-error={`,
            `    $handleError "IDENTITY_ERROR" "Failed to retrieve router identity" true;`,
            `    :error "Critical system error: Cannot access router identity";`,
            `}`,
            "",
            `$logOperation "info" "Starting OpenVPN Client Configuration Export Process";`,
            "",
            "# Step 1: Pre-flight checks - Verify OpenVPN server is configured",
            `$logOperation "info" "STEP 1: Pre-flight OpenVPN server verification";`,
            `:local ovpnServerExists false;`,
            `:local ovpnServerEnabled false;`,
            `:local ovpnServerCertificate "";`,
            `:local ovpnDetectionMethod "";`,
            `:local ovpnServerCount 0;`,
            "",
            "# Enhanced OpenVPN Server Configuration Detection with retry logic",
            `:local maxRetries 3;`,
            `:local retryDelay 2;`,
            `:for i from=1 to=$maxRetries do={`,
            `    $logOperation "info" ("OpenVPN detection attempt " . $i . " of " . $maxRetries);`,
            `    `,
            `    :do {`,
            `        # Method 1: Check if OpenVPN servers exist using count-only`,
            `        :do {`,
            `            :set ovpnServerCount [/interface ovpn-server server print count-only];`,
            `            :if ($ovpnServerCount > 0) do={`,
            `                :set ovpnServerExists true;`,
            `                :set ovpnDetectionMethod "count-only";`,
            `                $logOperation "info" ("Method 1 SUCCESS: Found " . $ovpnServerCount . " OpenVPN server(s) via count-only");`,
            `                `,
            `                # Try to get server status`,
            `                :do {`,
            `                    :local serverDisabled [/interface ovpn-server server get disabled];`,
            `                    :set ovpnServerEnabled (!$serverDisabled);`,
            `                    :if ($ovpnServerEnabled) do={`,
            `                        $logOperation "info" "OpenVPN server is enabled";`,
            `                        # Try to get certificate information`,
            `                        :do {`,
            `                            :set ovpnServerCertificate [/interface ovpn-server server get certificate];`,
            `                            $logOperation "info" ("OpenVPN server certificate: " . $ovpnServerCertificate);`,
            `                        } on-error={`,
            `                            $handleError "CERT_INFO_WARNING" "Could not retrieve OpenVPN server certificate info" false;`,
            `                            :set ovpnServerCertificate "unknown";`,
            `                        }`,
            `                    } else={`,
            `                        $handleError "SERVER_DISABLED" "OpenVPN server exists but is disabled" false;`,
            `                    }`,
            `                } on-error={`,
            `                    $handleError "SERVER_STATUS_WARNING" "Could not determine OpenVPN server status" false;`,
            `                    :set ovpnServerEnabled false;`,
            `                }`,
            `            } else={`,
            `                $logOperation "info" "Method 1: No OpenVPN servers found via count-only";`,
            `            }`,
            `        } on-error={`,
            `            $logOperation "info" "Method 1 failed: count-only command not available or failed";`,
            `        }`,
            `        `,
            `        # Method 2: Fallback - try basic interface existence check`,
            `        :if (!$ovpnServerExists) do={`,
            `            :do {`,
            `                :local ovpnMenu [/interface ovpn-server server find];`,
            `                :if ([:len $ovpnMenu] > 0) do={`,
            `                    :set ovpnServerExists true;`,
            `                    :set ovpnDetectionMethod "find-command";`,
            `                    :set ovpnServerCount [:len $ovpnMenu];`,
            `                    $logOperation "info" ("Method 2 SUCCESS: OpenVPN server detected via find command, count: " . $ovpnServerCount);`,
            `                    :set ovpnServerEnabled true;`,
            `                } else={`,
            `                    $logOperation "info" "Method 2: No OpenVPN servers found via find command";`,
            `                }`,
            `            } on-error={`,
            `                $logOperation "info" "Method 2 failed: find command not available or failed";`,
            `            }`,
            `        }`,
            `        `,
            `        # Method 3: Alternative detection method`,
            `        :if (!$ovpnServerExists) do={`,
            `            :do {`,
            `                # Try to access OpenVPN server menu directly`,
            `                :local ovpnTest [/interface ovpn-server server get [find]];`,
            `                :set ovpnServerExists true;`,
            `                :set ovpnDetectionMethod "direct-access";`,
            `                $logOperation "info" "Method 3 SUCCESS: OpenVPN server detected via direct access";`,
            `            } on-error={`,
            `                $logOperation "info" "Method 3 failed: direct access not available";`,
            `            }`,
            `        }`,
            `        `,
            `        # If detection successful, break retry loop`,
            `        :if ($ovpnServerExists) do={`,
            `            $logOperation "info" ("OpenVPN server detection successful using method: " . $ovpnDetectionMethod);`,
            `            :set i $maxRetries;`,
            `        }`,
            `        `,
            `    } on-error={`,
            `        $handleError "DETECTION_ERROR" ("OpenVPN detection attempt " . $i . " failed with error") false;`,
            `        :if ($i < $maxRetries) do={`,
            `            $logOperation "info" ("Retrying in " . $retryDelay . " seconds...");`,
            `            :delay $retryDelay;`,
            `        }`,
            `    }`,
            `}`,
            "",
            "# Evaluate OpenVPN server detection results",
            `:if (!$ovpnServerExists) do={`,
            `    $handleError "OVPN_SERVER_MISSING" "No OpenVPN server configuration detected after all attempts" false;`,
            `    $logOperation "warning" "This script works best with a configured OpenVPN server";`,
            `    $logOperation "info" "To configure OpenVPN server: /interface ovpn-server server set disabled=no certificate=your-cert";`,
            `    $logOperation "info" "Continuing with basic certificate export...";`,
            `} else={`,
            `    $logOperation "info" ("OpenVPN server detected successfully (Method: " . $ovpnDetectionMethod . ", Count: " . $ovpnServerCount . ")");`,
            `    :if (!$ovpnServerEnabled) do={`,
            `        $handleError "OVPN_SERVER_DISABLED" "OpenVPN server is configured but disabled" false;`,
            `        $logOperation "info" "To enable: /interface ovpn-server server set disabled=no";`,
            `    } else={`,
            `        $logOperation "info" "OpenVPN server is properly configured and enabled";`,
            `    }`,
            `}`,
            "",
            "# Step 2: Setup and use MikroTik Cloud DDNS with enhanced error handling",
            `$logOperation "info" "STEP 2: Setting up MikroTik Cloud DDNS with enhanced error handling";`,
            `:local serverAddress "";`,
            `:local useCloudDDNS false;`,
            `:local cloudRetryCount 0;`,
            `:local maxCloudRetries 3;`,
            `:local cloudRetryDelay 5;`,
            "",
            "# Enhanced Cloud DDNS setup with retry mechanism",
            `:for cloudAttempt from=1 to=$maxCloudRetries do={`,
            `    $logOperation "info" ("Cloud DDNS setup attempt " . $cloudAttempt . " of " . $maxCloudRetries);`,
            `    `,
            `    :do {`,
            `        :local cloudEnabled false;`,
            `        :local cloudDnsName "";`,
            `        `,
            `        # Check current Cloud DDNS status`,
            `        :do {`,
            `            :set cloudEnabled [/ip cloud get ddns-enabled];`,
            `            :set cloudDnsName [/ip cloud get dns-name];`,
            `            $logOperation "info" ("Cloud DDNS status - Enabled: " . $cloudEnabled . ", DNS Name: " . $cloudDnsName);`,
            `        } on-error={`,
            `            $handleError "CLOUD_ACCESS_ERROR" "Failed to access MikroTik Cloud configuration" false;`,
            `            :error "Cloud access failed";`,
            `        }`,
            `        `,
            `        # Enable Cloud DDNS if not enabled`,
            `        :if (!$cloudEnabled) do={`,
            `            $logOperation "info" "Enabling MikroTik Cloud DDNS...";`,
            `            :do {`,
            `                /ip cloud set ddns-enabled=yes;`,
            `                :delay $cloudRetryDelay;`,
            `                :set cloudEnabled [/ip cloud get ddns-enabled];`,
            `                :if ($cloudEnabled) do={`,
            `                    $logOperation "info" "MikroTik Cloud DDNS enabled successfully";`,
            `                } else={`,
            `                    :error "Failed to enable Cloud DDNS";`,
            `                }`,
            `            } on-error={`,
            `                $handleError "CLOUD_ENABLE_ERROR" "Failed to enable MikroTik Cloud DDNS" false;`,
            `                :error "Cloud enable failed";`,
            `            }`,
            `        }`,
            `        `,
            `        # Get DNS name with extended wait time`,
            `        :local dnsWaitAttempts 3;`,
            `        :for dnsWait from=1 to=$dnsWaitAttempts do={`,
            `            :set cloudDnsName [/ip cloud get dns-name];`,
            `            :if ([:len $cloudDnsName] > 0) do={`,
            `                :set dnsWait $dnsWaitAttempts;`,
            `            } else={`,
            `                :if ($dnsWait < $dnsWaitAttempts) do={`,
            `                    $logOperation "info" ("Waiting for DNS name assignment (attempt " . $dnsWait . "/" . $dnsWaitAttempts . ")...");`,
            `                    :delay 10;`,
            `                }`,
            `            }`,
            `        }`,
            `        `,
            `        # Evaluate DNS name result`,
            `        :if ([:len $cloudDnsName] > 0) do={`,
            `            :set serverAddress $cloudDnsName;`,
            `            :set useCloudDDNS true;`,
            `            $logOperation "info" ("SUCCESS: Using MikroTik Cloud DDNS: " . $serverAddress);`,
            `            :set cloudAttempt $maxCloudRetries;`,
            `        } else={`,
            `            $handleError "CLOUD_DNS_TIMEOUT" ("Cloud DDNS domain not available after waiting (attempt " . $cloudAttempt . ")") false;`,
            `            :if ($cloudAttempt < $maxCloudRetries) do={`,
            `                $logOperation "info" ("Retrying Cloud DDNS setup in " . $cloudRetryDelay . " seconds...");`,
            `                :delay $cloudRetryDelay;`,
            `            } else={`,
            `                :set serverAddress "";`,
            `                $handleError "CLOUD_DNS_FINAL_FAILURE" "Cloud DDNS domain not available after all attempts" false;`,
            `            }`,
            `        }`,
            `        `,
            `    } on-error={`,
            `        $handleError "CLOUD_SETUP_ERROR" ("Cloud DDNS setup failed on attempt " . $cloudAttempt) false;`,
            `        :if ($cloudAttempt < $maxCloudRetries) do={`,
            `            $logOperation "info" ("Retrying Cloud DDNS setup in " . $cloudRetryDelay . " seconds...");`,
            `            :delay $cloudRetryDelay;`,
            `        } else={`,
            `            :set serverAddress "";`,
            `            $handleError "CLOUD_FINAL_ERROR" "MikroTik Cloud DDNS setup failed after all attempts" false;`,
            `        }`,
            `    }`,
            `}`,
            "",
            "# Step 3: Enhanced certificate verification with detailed error handling",
            `$logOperation "info" "STEP 3: Enhanced certificate verification and preparation";`,
            `:local caCertName "";`,
            `:local clientCertName "";`,
            `:local serverCertName "";`,
            `:local certificatesReady false;`,
            `:local certCheckErrors [];`,
            `:local privateCertPrefix ($"ROUTER_IDENTITY" . "-CA");`,
            "",
            "# Check for PrivateCert generated certificates with detailed logging",
            `$logOperation "info" ("Checking for PrivateCert certificates with prefix: " . $privateCertPrefix);`,
            `:do {`,
            `    # Check CA Certificate with enhanced validation`,
            `    :do {`,
            `        :local caFind [/certificate find name=$privateCertPrefix authority=yes trusted=yes];`,
            `        :if ([:len $caFind] > 0) do={`,
            `            :set caCertName $privateCertPrefix;`,
            `            # Additional CA certificate validation`,
            `            :local caValid [/certificate get $caCertName trusted];`,
            `            :local caStatus [/certificate get $caCertName status];`,
            `            $logOperation "info" ("PrivateCert CA found: " . $caCertName . " (Status: " . $caStatus . ", Trusted: " . $caValid . ")");`,
            `        } else={`,
            `            $logOperation "info" "PrivateCert CA certificate not found";`,
            `        }`,
            `    } on-error={`,
            `        $handleError "CA_CERT_CHECK_ERROR" "Error checking PrivateCert CA certificate" false;`,
            `        :set ($certCheckErrors->[:len $certCheckErrors]) "CA certificate check failed";`,
            `    }`,
            `    `,
            `    # Check Server Certificate with enhanced validation`,
            `    :do {`,
            `        :local serverCertNameCheck ("Server@" . $privateCertPrefix);`,
            `        :local serverFind [/certificate find name=$serverCertNameCheck trusted=yes];`,
            `        :if ([:len $serverFind] > 0) do={`,
            `            :set serverCertName $serverCertNameCheck;`,
            `            # Additional server certificate validation`,
            `            :local serverValid [/certificate get $serverCertName trusted];`,
            `            :local serverStatus [/certificate get $serverCertName status];`,
            `            :local keyUsage [/certificate get $serverCertName key-usage];`,
            `            $logOperation "info" ("PrivateCert Server found: " . $serverCertName . " (Status: " . $serverStatus . ", Key Usage: " . $keyUsage . ")");`,
            `        } else={`,
            `            $logOperation "info" "PrivateCert Server certificate not found";`,
            `        }`,
            `    } on-error={`,
            `        $handleError "SERVER_CERT_CHECK_ERROR" "Error checking PrivateCert Server certificate" false;`,
            `        :set ($certCheckErrors->[:len $certCheckErrors]) "Server certificate check failed";`,
            `    }`,
            `    `,
            `    # Check Client Certificate with enhanced validation`,
            `    :do {`,
            `        :local clientCertNameCheck ("Client@" . $privateCertPrefix);`,
            `        :local clientFind [/certificate find name=$clientCertNameCheck trusted=yes];`,
            `        :if ([:len $clientFind] > 0) do={`,
            `            :set clientCertName $clientCertNameCheck;`,
            `            # Additional client certificate validation`,
            `            :local clientValid [/certificate get $clientCertName trusted];`,
            `            :local clientStatus [/certificate get $clientCertName status];`,
            `            :local keyUsage [/certificate get $clientCertName key-usage];`,
            `            $logOperation "info" ("PrivateCert Client found: " . $clientCertName . " (Status: " . $clientStatus . ", Key Usage: " . $keyUsage . ")");`,
            `        } else={`,
            `            $logOperation "info" "PrivateCert Client certificate not found";`,
            `        }`,
            `    } on-error={`,
            `        $handleError "CLIENT_CERT_CHECK_ERROR" "Error checking PrivateCert Client certificate" false;`,
            `        :set ($certCheckErrors->[:len $certCheckErrors]) "Client certificate check failed";`,
            `    }`,
            `    `,
            `    # Evaluate PrivateCert certificate completeness`,
            `    :if ([:len $caCertName] > 0 && [:len $serverCertName] > 0 && [:len $clientCertName] > 0) do={`,
            `        :set certificatesReady true;`,
            `        $logOperation "info" "SUCCESS: All PrivateCert certificates found and validated";`,
            `    } else={`,
            `        $logOperation "info" ("PrivateCert incomplete - CA: " . ([:len $caCertName] > 0 ? "OK" : "MISSING") . ", Server: " . ([:len $serverCertName] > 0 ? "OK" : "MISSING") . ", Client: " . ([:len $clientCertName] > 0 ? "OK" : "MISSING"));`,
            `    }`,
            `    `,
            `} on-error={`,
            `    $handleError "PRIVATECERT_CHECK_ERROR" "Critical error during PrivateCert certificate verification" false;`,
            `    :set ($certCheckErrors->[:len $certCheckErrors]) "PrivateCert verification failed";`,
            `}`,
            "",
            "# Alternative certificate detection if PrivateCert not available",
            `:if (!$certificatesReady) do={`,
            `    $logOperation "info" "PrivateCert not complete, checking for alternative certificates...";`,
            `    :do {`,
            `        # Enhanced alternative certificate search`,
            `        :local altCertCount 0;`,
            `        `,
            `        # Search for CA certificates`,
            `        :do {`,
            `            :local allCerts [/certificate find authority=yes trusted=yes];`,
            `            :set altCertCount [:len $allCerts];`,
            `            :if ($altCertCount > 0) do={`,
            `                :set caCertName [/certificate get ($allCerts->0) name];`,
            `                $logOperation "info" ("Found alternative CA certificate: " . $caCertName . " (from " . $altCertCount . " available)");`,
            `            } else={`,
            `                $logOperation "warning" "No alternative CA certificates found";`,
            `            }`,
            `        } on-error={`,
            `            $handleError "ALT_CA_SEARCH_ERROR" "Error searching for alternative CA certificates" false;`,
            `        }`,
            `        `,
            `        # Search for server certificates`,
            `        :do {`,
            `            :local serverCerts [/certificate find key-usage~"tls-server" trusted=yes];`,
            `            :if ([:len $serverCerts] > 0) do={`,
            `                :set serverCertName [/certificate get ($serverCerts->0) name];`,
            `                $logOperation "info" ("Found alternative server certificate: " . $serverCertName);`,
            `            } else={`,
            `                $logOperation "warning" "No alternative server certificates found";`,
            `            }`,
            `        } on-error={`,
            `            $handleError "ALT_SERVER_SEARCH_ERROR" "Error searching for alternative server certificates" false;`,
            `        }`,
            `        `,
            `        # Search for client certificates`,
            `        :do {`,
            `            :local clientCerts [/certificate find key-usage~"tls-client" trusted=yes];`,
            `            :if ([:len $clientCerts] > 0) do={`,
            `                :set clientCertName [/certificate get ($clientCerts->0) name];`,
            `                $logOperation "info" ("Found alternative client certificate: " . $clientCertName);`,
            `            } else={`,
            `                $logOperation "warning" "No alternative client certificates found";`,
            `            }`,
            `        } on-error={`,
            `            $handleError "ALT_CLIENT_SEARCH_ERROR" "Error searching for alternative client certificates" false;`,
            `        }`,
            `        `,
            `        # Evaluate alternative certificate completeness`,
            `        :if ([:len $caCertName] > 0 && [:len $serverCertName] > 0) do={`,
            `            :set certificatesReady true;`,
            `            $logOperation "info" "SUCCESS: Alternative certificates found and ready";`,
            `        } else={`,
            `            $logOperation "warning" "Alternative certificates incomplete";`,
            `        }`,
            `        `,
            `    } on-error={`,
            `        $handleError "ALT_CERT_CHECK_ERROR" "Error during alternative certificate verification" false;`,
            `    }`,
            `}`,
            "",
            "# Step 4: Enhanced certificate file verification",
            `$logOperation "info" "STEP 4: Enhanced certificate file verification";`,
            `:local caFileExists false;`,
            `:local clientCrtExists false;`,
            `:local clientKeyExists false;`,
            `:local fileCheckErrors [];`,
            `:local fileDetails [];`,
            "",
            ...
            `        :local fileSize [/file get $file size];`,
            `        :set totalFiles ($totalFiles + 1);`,
            `        `,
            `        :if ($fileName = "ca_certificate.crt") do={`,
            `            :set caFileExists true;`,
            `            :set certFiles ($certFiles + 1);`,
            `            :set ($fileDetails->[:len $fileDetails]) ("ca_certificate.crt: " . $fileSize . " bytes");`,
            `            $logOperation "info" ("Found ca_certificate.crt (" . $fileSize . " bytes)");`,
            `        }`,
            `        :if ($fileName = "client_bundle.crt") do={`,
            `            :set clientCrtExists true;`,
            `            :set certFiles ($certFiles + 1);`,
            `            :set ($fileDetails->[:len $fileDetails]) ("client_bundle.crt: " . $fileSize . " bytes");`,
            `            $logOperation "info" ("Found client_bundle.crt (" . $fileSize . " bytes)");`,
            `        }`,
            `        :if ($fileName = "client_bundle.key") do={`,
            `            :set clientKeyExists true;`,
            `            :set certFiles ($certFiles + 1);`,
            `            :set ($fileDetails->[:len $fileDetails]) ("client_bundle.key: " . $fileSize . " bytes");`,
            `            $logOperation "info" ("Found client_bundle.key (" . $fileSize . " bytes)");`,
            `        }`,
            `    }`,
            `    `,
            `    $logOperation "info" ("File scan complete: " . $totalFiles . " total files, " . $certFiles . " certificate files found");`,
            `    `,
            `} on-error={`,
            `    $handleError "FILE_SCAN_ERROR" "Error during certificate file scan" false;`,
            `    :set ($fileCheckErrors->[:len $fileCheckErrors]) "File system scan failed";`,
            `}`,
            "",
            "# Evaluate file availability",
            `:if ($caFileExists && $clientCrtExists && $clientKeyExists) do={`,
            `    $logOperation "info" "SUCCESS: All required certificate files found (exported by ExportCert function)";`,
            `    :set certificatesReady true;`,
            `} else={`,
            `    $handleError "CERT_FILES_INCOMPLETE" "Some certificate files are missing" false;`,
            `    :if (!$caFileExists) do={`,
            `        $logOperation "error" "Missing: ca_certificate.crt";`,
            `        :set ($fileCheckErrors->[:len $fileCheckErrors]) "ca_certificate.crt missing";`,
            `    }`,
            `    :if (!$clientCrtExists) do={`,
            `        $logOperation "error" "Missing: client_bundle.crt";`,
            `        :set ($fileCheckErrors->[:len $fileCheckErrors]) "client_bundle.crt missing";`,
            `    }`,
            `    :if (!$clientKeyExists) do={`,
            `        $logOperation "error" "Missing: client_bundle.key";`,
            `        :set ($fileCheckErrors->[:len $fileCheckErrors]) "client_bundle.key missing";`,
            `    }`,
            `    $logOperation "warning" "Please run ExportCert function first to export certificate files";`,
            `    :set certificatesReady false;`,
            `}`,
            "",
            "# Step 5: Enhanced OpenVPN configuration export with comprehensive error handling",
            `$logOperation "info" "STEP 5: Enhanced OpenVPN client configuration generation";`,
            `:local exportSuccess false;`,
            `:local exportErrors [];`,
            "",
            `:do {`,
            `    :if ([:len $caCertName] > 0 && $certificatesReady) do={`,
            `        $logOperation "info" "Attempting OpenVPN client configuration export...";`,
            `        `,
            `        # Pre-export validation`,
            `        :if ([:len $serverAddress] = 0) do={`,
            `            $handleError "EXPORT_VALIDATION_ERROR" "Server address is empty - cannot create valid configuration" false;`,
            `            :set ($exportErrors->[:len $exportErrors]) "Empty server address";`,
            `        }`,
            `        `,
            `        # Attempt export with detailed error handling`,
            `        :do {`,
            `            # Note: According to MikroTik documentation, the server parameter is required in RouterOS 7.17+`,
            `            # Also, require-client-certificate must be enabled for export to work`,
            `            :local serverName "";`,
            `            :local serverFound false;`,
            `            `,
            `            # Find and validate OpenVPN server configuration`,
            `            :do {`,
            `                :foreach serverEntry in=[/interface ovpn-server server find] do={`,
            `                    :set serverName [/interface ovpn-server server get $serverEntry name];`,
            `                    :local requireClientCert [/interface ovpn-server server get $serverEntry require-client-certificate];`,
            `                    `,
            `                    :if ($requireClientCert) do={`,
            `                        :set serverFound true;`,
            `                        $logOperation "info" ("Found suitable server: " . $serverName . " with require-client-certificate enabled");`,
            `                    } else={`,
            `                        $logOperation "warning" ("Server " . $serverName . " found but require-client-certificate is disabled");`,
            `                        # Enable require-client-certificate for export to work`,
            `                        /interface ovpn-server server set $serverEntry require-client-certificate=yes;`,
            `                        :set serverFound true;`,
            `                        $logOperation "info" ("Enabled require-client-certificate for server: " . $serverName);`,
            `                    }`,
            `                    `,
            `                    :if ($serverFound) do={`,
            `                        # Try RouterOS 7.17+ syntax with server parameter`,
            ...
            `                        $logOperation "info" ("SUCCESS: OpenVPN client configuration exported for server: " . $serverName);`,
            `                        :error "Export completed successfully";`,
            `                    }`,
            `                }`,
            `                `,
            `                :if (!$serverFound) do={`,
            `                    :error "No suitable OpenVPN server found or configured";`,
            `                }`,
            `                `,
            `            } on-error={`,
            `                :if ($exportSuccess) do={`,
            `                    # Export was successful, this is expected`,
            `                } else={`,
            `                    # Try fallback for older RouterOS versions`,
            `                    $logOperation "info" "Trying legacy export command (RouterOS < 7.17)";`,
            ...
            `                    :set exportSuccess true;`,
            `                    $logOperation "info" "SUCCESS: OpenVPN client configuration exported using legacy command";`,
            `                }`,
            `            }`,
            `            `,
            `            # Verify export result`,
            `            :local ovpnFileExists false;`,
            `            :foreach file in=[/file find] do={`,
            `                :local fileName [/file get $file name];`,
            `                :if ($fileName ~ "\\.ovpn$") do={`,
            `                    :set ovpnFileExists true;`,
            `                    :local fileSize [/file get $file size];`,
            `                    $logOperation "info" ("Generated OpenVPN file: " . $fileName . " (" . $fileSize . " bytes)");`,
            `                }`,
            `            }`,
            `            `,
            `            :if (!$ovpnFileExists) do={`,
            `                $handleError "EXPORT_VERIFICATION_ERROR" "OpenVPN export completed but no .ovpn file found" false;`,
            `                :set exportSuccess false;`,
            `            }`,
            `            `,
            `        } on-error={`,
            `            $handleError "EXPORT_EXECUTION_ERROR" "OpenVPN export command failed to execute" false;`,
            `            :set ($exportErrors->[:len $exportErrors]) "Export command execution failed";`,
            `            :set exportSuccess false;`,
            `        }`,
            `        `,
            `    } else={`,
            `        $handleError "EXPORT_PREREQ_ERROR" "Prerequisites not met for OpenVPN export" false;`,
            `        :if ([:len $caCertName] = 0) do={`,
            `            :set ($exportErrors->[:len $exportErrors]) "No CA certificate available";`,
            `        }`,
            `        :if (!$certificatesReady) do={`,
            `            :set ($exportErrors->[:len $exportErrors]) "Certificate files not ready";`,
            `        }`,
            `        $logOperation "warning" "Creating basic configuration template instead";`,
            `    }`,
            `    `,
            `} on-error={`,
            `    $handleError "EXPORT_CRITICAL_ERROR" "Critical error during OpenVPN configuration export" true;`,
            `    :set ($exportErrors->[:len $exportErrors]) "Critical export failure";`,
            `    $logOperation "error" "Export process failed completely";`,
            `}`,
            "",
            "# Step 6: Comprehensive final summary with detailed error reporting",
            `$logOperation "info" "STEP 6: Generating comprehensive summary and error report";`,
            `:local endTime [/system clock get time];`,
            `:local endDate [/system clock get date];`,
            `:local executionTime ("Started: " . $startDate . " " . $startTime . ", Ended: " . $endDate . " " . $endTime);`,
            "",
            `:put "";`,
            `:put "=== OPENVPN CLIENT CONFIGURATION EXPORT SUMMARY ===";`,
            `:put $executionTime;`,
            `:put ("Total Errors: " . $errorCount . ", Total Warnings: " . $warningCount);`,
            "",
            "# Server Address Information",
            `:put ("Server Address: " . ([:len $serverAddress] > 0 ? $serverAddress : "NOT AVAILABLE"));`,
            `:if ($useCloudDDNS) do={`,
            `    :put "Address Type: MikroTik Cloud DDNS (dynamic)";`,
            `} else={`,
            `    :put "Address Type: Manual configuration required";`,
            `}`,
            "",
            "# OpenVPN Server Status",
            `:put ("OpenVPN Server: " . ($ovpnServerExists ? "DETECTED" : "NOT FOUND"));`,
            `:if ($ovpnServerExists) do={`,
            `    :put ("Server Status: " . ($ovpnServerEnabled ? "ENABLED" : "DISABLED"));`,
            `    :put ("Detection Method: " . $ovpnDetectionMethod);`,
            `    :put ("Server Count: " . $ovpnServerCount);`,
            `    :if ([:len $ovpnServerCertificate] > 0) do={`,
            `        :put ("Server Certificate: " . $ovpnServerCertificate);`,
            `    }`,
            `}`,
            "",
            "# Certificate Status",
            `:put ("Certificates Ready: " . ($certificatesReady ? "YES" : "NO"));`,
            `:if ($certificatesReady) do={`,
            `    :put "Certificate Details:";`,
            `    :if ([:len $caCertName] > 0) do={`,
            `        :put ("  - CA Certificate: " . $caCertName);`,
            `    }`,
            `    :if ([:len $serverCertName] > 0) do={`,
            `        :put ("  - Server Certificate: " . $serverCertName);`,
            `    }`,
            `    :if ([:len $clientCertName] > 0) do={`,
            `        :put ("  - Client Certificate: " . $clientCertName);`,
            `    }`,
            `    :put "Certificate Files:";`,
            `    :foreach detail in=$fileDetails do={`,
            `        :put ("  - " . $detail);`,
            `    }`,
            `} else={`,
            `    :put "Certificate Issues Detected - Run ExportCert function first";`,
            `}`,
            "",
            "# Export Status",
            `:put ("Configuration Export: " . ($exportSuccess ? "SUCCESS" : "FAILED"));`,
            `:if (!$exportSuccess && [:len $exportErrors] > 0) do={`,
            `    :put "Export Errors:";`,
            `    :foreach error in=$exportErrors do={`,
            `        :put ("  - " . $error);`,
            `    }`,
            `}`,
            "",
            ...
            `    :put "";`,
            `    :put "=== CRITICAL ERRORS ===";`,
            `    :foreach error in=$criticalErrors do={`,
            `        :put $error;`,
            `    }`,
            `}`,
            "",
            `:if ($warningCount > 0) do={`,
            `    :put "";`,
            `    :put "=== WARNINGS ===";`,
            ...
            `:put "";`,
            `:put "=== OPERATION LOG ===";`,
            `:foreach logEntry in=$operationLog do={`,
            `    :put $logEntry;`,
            `}`,
            "",
            "# Final Instructions",
            ...
            `:put "=== NEXT STEPS ===";`,
            `:if ($exportSuccess) do={`,
            `    :put "1. Download the generated .ovpn file from Files menu";`,
            `    :put "2. Import the configuration into your OpenVPN client";`,
            `    :put "3. Connect to the VPN";`,
            `} else={`,
            `    :put "1. Review and resolve the errors listed above";`,
            `    :put "2. Run ExportCert function if certificate files are missing";`,
            `    :put "3. Ensure OpenVPN server is properly configured";`,
            `    :put "4. Verify MikroTik Cloud DDNS is working";`,
            `    :put "5. Re-run this script after resolving issues";`,
            `}`,
            "",
            `:put "";`,
            `:put "=== TROUBLESHOOTING COMMANDS ===";`,
            `:put "- Check OpenVPN server: /interface ovpn-server server print detail";`,
            `:put "- Check server status: /interface ovpn-server server print where disabled=no";`,
            `:put "- Check require-client-certificate: /interface ovpn-server server get [find] require-client-certificate";`,
            `:put "- Check certificates: /certificate print detail";`,
            `:put "- Check Cloud DDNS: /ip cloud print";`,
            `:put "- Check files: /file print where name~\\.ovpn";`,
            `:put "- Check logs: /log print where topics~ovpn";`,
            `:put "- Test export manually: /interface ovpn-server server export-client-configuration server=myServer server-address=1.1.1.1 ca-certificate=ca.crt client-certificate=client.crt client-cert-key=client.key";`,
            "",
            "# Final logging",
            `$logOperation "info" ("Export process completed with " . $errorCount . " errors and " . $warningCount . " warnings");`,
            `$logOperation "info" ("Export success: " . $exportSuccess);`,
            `$logOperation "info" "=== OPENVPN CLIENT CONFIGURATION EXPORT COMPLETED ===";`,
            `:put "=== OpenVPN Client Configuration Export Process Complete ===";`
        ]
    };
