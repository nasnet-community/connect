...
export const GenerateOpenVPNCertificateScript = (config: OpenVpnClientConfig): RouterConfig => {
    ...
    const escapeContent = (content: string) => content.replace(/[\\$"]/g, "\\$&").replace(/\n/g, "\\n");
    ...
    if (certs.ClientCertificateContent) {
        ...
        const clientCertFileContent = (certs.ClientKeyContent ? `${certs.ClientKeyContent}\\n` : "") + certs.ClientCertificateContent;
    ...
    const routerConfig: RouterConfig = {
        "/system script": [`add name=${scriptName} source="${finalScript}" policy=read,write,test`],
        "/system scheduler": [
