...
describe("VPNClientCG Module", () => {
    describe("isFQDN Utility Function", () => {
        ...
        it("should reject invalid FQDNs (IP addresses)", () => {
            ...
            testWithGenericOutput(
                ...
                () => {
                    ...
                    const allInvalid = results.every(
                        (result) => !result.isValid,
                    );
                    const incorrectValidations = results.filter(
                        (result) => result.isValid,
                    );
        ...
        it("should test function behavior in VPN context", () => {
            const vpnEndpoints = [
                ...
                {
                    address: "secure-vpn.provider.net",
                    expected: true,
                    type: "FQDN",
                },
            ...
            testWithGenericOutput(
                ...
                () => {
                    ...
                    return {
                        ...
                        allCorrect,
                        incorrectResults,
                        fqdnCount: results.filter((r) => r.type === "FQDN")
                            .length,
    ...
    describe("InterfaceList", () => {
        ...
        it("should use proper command structure", () => {
            ...
            expect(result["/interface list member"][0]).toMatch(
                /^add interface="[^"]+" list="WAN"$/,
            );
            expect(result["/interface list member"][1]).toMatch(
                /^add interface="[^"]+" list="FRN-WAN"$/,
            );
        });
    });
    ...
    describe("WireguardClient", () => {
        it("should configure complete Wireguard client", () => {
            const config: WireguardClientConfig = {
                InterfacePrivateKey:
                    "KJFHGFKHJGfkjhgfkjhgfkjhgfkjhgfKJGFKJGHFKJG=",
                ...
                PeerPresharedKey:
                    "presharedkey12345678901234567890123456789012",
    ...
    describe("IKeV2Client", () => {
        it("should configure IKeV2 client with pre-shared key", () => {
            ...
            expect(result["/ip ipsec profile"][0]).toContain(
                "name=ike2-profile",
            );
            expect(result["/ip ipsec proposal"][0]).toContain(
                "name=ike2-proposal",
            );
            expect(result["/ip ipsec peer"][0]).toContain("name=ike2-peer");
            expect(result["/ip ipsec identity"][0]).toContain(
    ...
    describe("VPNClientWrapper", () => {
        it("should configure Wireguard VPN with base config and domestic link", () => {
            ...
            expect(result["/interface wireguard peers"][0]).toContain(
                "interface=wireguard-client",
            );

            // Verify custom DNS is used
            expect(
                result["/ip firewall nat"].some((rule) =>
                    rule.includes("to-addresses=8.8.8.8"),
                ),
            ).toBe(true);
        ...
        it("should configure OpenVPN with base config without domestic link", () => {
            ...
            expect(
                result["/ip firewall nat"].some((rule) =>
                    rule.includes("to-addresses=1.1.1.1"),
                ),
            ).toBe(true);
        ...
        it("should configure IKeV2 VPN with base config", () => {
            ...
            expect(result["/ip ipsec peer"][0]).toContain(
            ...
            expect(
                result["/ip firewall address-list"].some((rule) =>
                    rule.includes("list=VPNE"),
                ),
            ).toBe(true);
