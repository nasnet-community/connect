...
export const DNSForwarders = (DNSConfig: DNSConfig, baseNetworks: Networks[] = []): RouterConfig => {
  ...
  baseNetworks.forEach((network) => {
    ...
    if (dnsServer && dnsServer.trim() !== "") {
      ...
      if (network === "Domestic" && DOH?.domain) {
        const dohDomain = DOH.domain.startsWith('http') ? DOH.domain : `https://${DOH.domain}`;
        const dohUrl = dohDomain.endsWith('/dns-query') ? dohDomain : `${dohDomain}/dns-query`;
...
export const DNSmDNSRepeater = (baseNetworks: Networks[] = []): RouterConfig => {
  const config: RouterConfig = {
    "/ip dns": []
  };
  ...
  const bridgeInterfaces: string[] = baseNetworks.map(network => `LANBridge${networkNameMapping[network]}`);
