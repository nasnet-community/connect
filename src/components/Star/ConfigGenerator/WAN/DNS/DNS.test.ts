...
describe("DNS Configuration Tests", () => {
  describe("DNSForwarders", () => {
    it("should create DNS forwarders for configured networks", () => {
      ...
      const baseNetworks: Networks[] = ["Foreign", "VPN", "Domestic", "Split"];
      ...
      expect(result["/ip dns forwarders"]).toContain("add name=FOREIGN dns-servers=1.1.1.1");
      expect(result["/ip dns forwarders"]).toContain("add name=VPN dns-servers=8.8.8.8");
      expect(result["/ip dns forwarders"]).toContain("add name=DOMESTIC dns-servers=178.22.122.100 doh-servers=https://cloudflare-dns.com/dns-query verify-doh-cert=yes");
      expect(result["/ip dns forwarders"]).toContain("add name=SPLIT dns-servers=4.2.2.4");
    ...
    it("should only create forwarders for networks with DNS servers", () => {
      ...
      expect(result["/ip dns forwarders"]).toContain("add name=FOREIGN dns-servers=1.1.1.1");
      expect(result["/ip dns forwarders"]).toContain("add name=DOMESTIC dns-servers=178.22.122.100");
      
      // Should not create VPN forwarder
      expect(result["/ip dns forwarders"].some(cmd => cmd.includes("VPN"))).toBe(false);
    ...
    it("should only configure DoH for Domestic network", () => {
      ...
      const baseNetworks: Networks[] = ["Foreign", "VPN", "Split", "Domestic"];
      ...
      const nonDomesticForwarders = result["/ip dns forwarders"].filter(cmd => !cmd.includes("DOMESTIC"));
  ...
  describe("DNSmDNSRepeater", () => {
    it("should create mDNS repeater configuration for base networks", () => {
      ...
      expect(result["/ip dns"]).toContain("set mdns-repeat-ifaces=LANBridgeFRN,LANBridgeVPN,LANBridgeDOM,LANBridgeSplit");
    });
    ...
    it("should handle single network", () => {
      ...
      expect(result["/ip dns"]).toBeDefined();
      expect(result["/ip dns"]).toContain("set mdns-repeat-ifaces=LANBridgeFRN");
    });
    ...
    it("should create correct bridge interface names", () => {
      ...
      expect(result["/ip dns"]).toContain("set mdns-repeat-ifaces=LANBridgeDOM,LANBridgeVPN");
    });
  });
});
