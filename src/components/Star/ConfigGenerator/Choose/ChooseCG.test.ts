    ...
    it("should include DNS static entries for .ir TLD and local addresses", () => {
      ...
      testWithGenericOutput(
        ...
        () => {
          ...
          const hasLocalDNS = staticEntries.some((cmd) =>
      ...
      testWithGenericOutput(
        ...
        () => {
          ...
          return {
            ...
            hasDNSServers:
              result["/ip dns"].some((cmd) => cmd.includes("servers=")) ||
              false,
            hasDoHServer:
              result["/ip dns"].some((cmd) =>
                cmd.includes("use-doh-server="),
              ) || false,
            hasDoHMangleRule: mangleRules.some((cmd) =>
    ...
    it("should validate DometicBase contains correct bridge configuration", () => {
      ...
      testWithGenericOutput(
        ...
        () => {
          ...
          const hasBridge = bridgeCommands.some((cmd) =>
            cmd.includes("name=LANBridgeDOM"),
          );
          const hasAddress = addressCommands.some((cmd) =>
