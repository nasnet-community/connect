...
export const IPIPInterfaceWrapper = (
...
export const EoipInterfaceWrapper = (
...
export const GreInterfaceWrapper = (
    gre: GreTunnelConfig,
    subnet?: SubnetConfig,
): RouterConfig => {
    const configs: RouterConfig[] = [];
...
export const VxlanInterfaceWrapper = (
...
export const TunnelWrapper = (
    ...
    if (tunnel.IPIP && tunnel.IPIP.length > 0) {
        tunnel.IPIP.forEach((ipip) => {
            const matchedSubnet = findSubnetByName(
                ipip.name,
                tunnelSubnets?.IPIP,
            );
