...
import type { Wireless, WirelessConfig as WirelessConfigType, MultiMode } from '~/components/Star/StarContext/LANType';
...
describe('Wireless Helper Function Tests', () => {
  ...
  describe('GetNetworks', () => {
    it('should return all networks if they exist', () => {
      ...
      expect(GetNetworks(multiMode)).toEqual(["Split", "Foreign", "Domestic", "VPN"]);
    });
  ...
  describe('StationMode', () => {
    it('should generate station mode command with credentials', () => {
      ...
      expect(commands[0]).toContain('set [ find default-name=wifi2 ] comment=ForeignWAN');
  ...
  describe('Slave', () => {
    ...
    describe('Basic Slave Interface Generation', () => {
      it('should generate basic slave interface configuration for Domestic network', () => {
        ...
        const result = Slave('Domestic' as Networks, '5' as Band, baseWirelessConfig);
        ...
        expect(commands).toContain('master-interface=[ find default-name=wifi1 ]');
        ...
        expect(commands).toContain('security.authentication-types=wpa2-psk,wpa3-psk');
      ...
      it('should generate slave interface for 2.4GHz band', () => {
        ...
        const result = Slave('Foreign' as Networks, '2.4' as Band, baseWirelessConfig);
        const commands = result['/interface wifi'].join(' ');
      ...
      it('should generate slave interface for VPN network', () => {
        ...
        const result = Slave('VPN' as Networks, '5' as Band, baseWirelessConfig);
        const commands = result['/interface wifi'].join(' ');
        expect(commands).toContain('master-interface=[ find default-name=wifi1 ]');
      ...
      it('should generate slave interface for Split network', () => {
        ...
        const result = Slave('Split' as Networks, '2.4' as Band, baseWirelessConfig);
        const commands = result['/interface wifi'].join(' ');
        expect(commands).toContain('master-interface=[ find default-name=wifi2 ]');
    ...
    describe('Split Band SSID Configuration', () => {
      it('should generate slave interface with split band enabled for 2.4GHz', () => {
        ...
        const result = Slave('Foreign' as Networks, '2.4' as Band, splitBandConfig);
        const commands = result['/interface wifi'].join(' ');
      ...
      it('should generate slave interface with split band enabled for 5GHz', () => {
        ...
        const result = Slave('Domestic' as Networks, '5' as Band, splitBandConfig);
        const commands = result['/interface wifi'].join(' ');
    ...
    describe('Complex Password and SSID Handling', () => {
      it('should handle special characters in SSID and password', () => {
        ...
        const result = Slave('Foreign' as Networks, '2.4' as Band, specialConfig);
        const commands = result['/interface wifi'].join(' ');
      ...
      it('should handle long SSID and password strings', () => {
        ...
        const commands = result['/interface wifi'].join(' ');
        expect(commands).toContain('.ssid="VeryLongSSIDNameForTestingPurposesOnly 5"');
        expect(commands).toContain('.passphrase="VeryLongPasswordForTestingComplexConfigurationHandling123456789"');
        expect(commands).toContain('configuration.hide-ssid=yes');
        validateRouterConfig(result, ['/interface wifi']);
      });
    ...
    describe('Master Interface Integration', () => {
      it('should correctly reference master interface for all networks', () => {
        ...
        for (const testCase of testCases) {
          testWithOutput(
            ...
            testCase,
            () => Slave(testCase.Network, testCase.Band as Band, baseWirelessConfig)
          );

          const result = Slave(testCase.Network, testCase.Band as Band, baseWirelessConfig);
          const commands = result['/interface wifi'].join(' ');
          expect(commands).toContain('add configuration.mode=ap');
          expect(commands).toContain(`name="wifi${testCase.Band}-${testCase.Network}LAN"`);
          ...
          expect(commands).toContain(`master-interface=[ find default-name=${expectedMaster} ]`);
      ...
      it('should handle different WAN link configurations', () => {
        ...
        const result = Slave('Domestic' as Networks, '2.4' as Band, baseWirelessConfig);
        const commands = result['/interface wifi'].join(' ');
        expect(commands).toContain('master-interface=[ find default-name=wifi2 ]');
    ...
    describe('Command Structure Validation', () => {
      it('should generate complete and properly formatted command', () => {
        ...
        const result = Slave('Foreign' as Networks, '5' as Band, completeConfig);
        const commands = result['/interface wifi'].join(' ');
      ...
      it('should maintain proper command order and syntax', () => {
        ...
        const result = Slave('VPN' as Networks, '2.4' as Band, baseWirelessConfig);
        const commands = result['/interface wifi'].join(' ');
    ...
    describe('Edge Cases and Error Scenarios', () => {
      ...
      it('should handle both bands consistently', () => {
        ...
        for (const band of bands) {
          results[band] = Slave('Domestic' as Networks, band, baseWirelessConfig);
        }
        ...
        expect(commands24).toContain('master-interface=[ find default-name=wifi2 ]');
        expect(commands24).toContain('name="wifi2.4-DomesticLAN"');
        expect(commands5).toContain('master-interface=[ find default-name=wifi1 ]');
  ...
  describe('Master', () => {
    ...
    describe('Basic Master Interface Generation', () => {
      it('should generate basic master interface configuration for Foreign network', () => {
        ...
        const result = Master('Foreign' as Networks, '2.4' as Band, baseMasterConfig);
        const commands = result['/interface wifi'].join(' ');
      ...
      it('should generate master interface for 5GHz band', () => {
        ...
        const result = Master('Domestic' as Networks, '5' as Band, baseMasterConfig);
        const commands = result['/interface wifi'].join(' ');
      ...
      it('should generate master interface for VPN network', () => {
        ...
        const result = Master('VPN' as Networks, '2.4' as Band, baseMasterConfig);
        const commands = result['/interface wifi'].join(' ');
        expect(commands).toContain('set [ find default-name=wifi2 ]');
      ...
      it('should generate master interface for Split network', () => {
        ...
        const result = Master('Split' as Networks, '5' as Band, baseMasterConfig);
        const commands = result['/interface wifi'].join(' ');
        expect(commands).toContain('set [ find default-name=wifi1 ]');
    ...
    describe('Hidden SSID Configuration', () => {
      it('should generate master interface with hidden SSID', () => {
        ...
        const result = Master('Foreign' as Networks, '2.4' as Band, hiddenMasterConfig);
        const commands = result['/interface wifi'].join(' ');
        expect(commands).toContain('configuration.hide-ssid=yes');
        expect(commands).toContain('.ssid="MasterNetwork"');
        validateRouterConfig(result, ['/interface wifi']);
      ...
      it('should generate master interface with visible SSID', () => {
        ...
        const result = Master('Domestic' as Networks, '5' as Band, visibleMasterConfig);
        const commands = result['/interface wifi'].join(' ');
        expect(commands).toContain('configuration.hide-ssid=no');
        validateRouterConfig(result, ['/interface wifi']);
      });
    ...
    describe('Split Band SSID Configuration', () => {
      it('should generate master interface with split band enabled for 2.4GHz', () => {
        ...
        const result = Master('Foreign' as Networks, '2.4' as Band, splitBandMasterConfig);
        const commands = result['/interface wifi'].join(' ');
      ...
      it('should generate master interface with split band enabled for 5GHz', () => {
        ...
        const result = Master('VPN' as Networks, '5' as Band, splitBandMasterConfig);
        const commands = result['/interface wifi'].join(' ');
      ...
      it('should generate master interface with unified SSID when split band disabled', () => {
        ...
        const result = Master('Split' as Networks, '2.4' as Band, unifiedMasterConfig);
        const commands = result['/interface wifi'].join(' ');
    ...
    describe('Complex Password and SSID Handling', () => {
      it('should handle special characters in SSID and password', () => {
        ...
        const result = Master('Foreign' as Networks, '5' as Band, specialMasterConfig);
        const commands = result['/interface wifi'].join(' ');
      ...
      it('should handle long SSID and password strings', () => {
        const longMasterConfig: WirelessConfigType = { 
          ...
          Password: 'VeryLongMasterPasswordForTestingComplexConfigurationHandling123456789', 
          isHide: true, 
          SplitBand: true, 
          isDisabled: false 
        };
        ...
        const result = Master('Domestic' as Networks, '2.4' as Band, longMasterConfig);
        const commands = result['/interface wifi'].join(' ');
        expect(commands).toContain('.ssid="VeryLongMasterSSIDNameForTestingPurposesOnly 2.4"');
        expect(commands).toContain('.passphrase="VeryLongMasterPasswordForTestingComplexConfigurationHandling123456789"');
        expect(commands).toContain('configuration.hide-ssid=yes');
        validateRouterConfig(result, ['/interface wifi']);
      });
      ...
      it('should handle minimal configurations', () => {
        ...
        const result = Master('VPN' as Networks, '5' as Band, minimalMasterConfig);
        const commands = result['/interface wifi'].join(' ');
    ...
    describe('MikroTik WiFi Configuration Standards', () => {
      it('should generate master interface following MikroTik WiFi documentation', () => {
        ...
        testWithOutput(
          ...
          () => Master('Foreign' as Networks, '2.4' as Band, mikrotikStandardConfig)
        );

        const result = Master('Foreign' as Networks, '2.4' as Band, mikrotikStandardConfig);
        const commands = result['/interface wifi'].join(' ');
        ...
        expect(commands).toContain('security.authentication-types=wpa2-psk,wpa3-psk');
        expect(commands).toContain('.passphrase="diceware makes good passwords"');
        expect(commands).toContain('disabled=no');
        validateRouterConfig(result, ['/interface wifi']);
      });
    ...
    describe('Edge Cases and Error Scenarios', () => {
      ...
      it('should generate consistent configuration structure', () => {
        ...
        const result = Master('Domestic' as Networks, '2.4' as Band, baseMasterConfig);
  ...
  describe('WirelessBridgePortsSingle', () => {
    describe('Domestic Link Configuration', () => {
      ...
      it('should configure both 2.4GHz and 5GHz interfaces for split LAN', () => {
        ...
        expect(commands.some((cmd: string) => cmd.includes('wifi2.4-SplitLAN'))).toBe(true);
        expect(commands.some((cmd: string) => cmd.includes('wifi5-SplitLAN'))).toBe(true);
        validateRouterConfig(result, ['/interface bridge port']);
      });
    });
    ...
    describe('Non-Domestic Link Configuration', () => {
      ...
      it('should configure both 2.4GHz and 5GHz interfaces for VPN LAN', () => {
        ...
        expect(commands.some((cmd: string) => cmd.includes('wifi2.4-VPNLAN'))).toBe(true);
        expect(commands.some((cmd: string) => cmd.includes('wifi5-VPNLAN'))).toBe(true);
        validateRouterConfig(result, ['/interface bridge port']);
      });
    });
    ...
    describe('Router Config Structure Validation', () => {
      it('should return valid RouterConfig structure for domestic link', () => {
        ...
        expect(result['/interface bridge port'].every((cmd: string) => typeof cmd === 'string')).toBe(true);
        validateRouterConfig(result, ['/interface bridge port']);
      });
  ...
  describe('WirelessBridgePortsMulti', () => {
    describe('MultiMode Network Configurations', () => {
      it('should generate bridge ports for all networks in multimode', () => {
        ...
        expect(result['/interface bridge port']).toEqual(expect.arrayContaining(expectedPorts));
    ...
    describe('Network Naming Convention Validation', () => {
      ...
      it('should configure both 2.4GHz and 5GHz for each network', () => {
        ...
        expect(commands.filter((cmd: string) => cmd.includes('wifi2.4-FRNLAN'))).toHaveLength(1);
        expect(commands.filter((cmd: string) => cmd.includes('wifi5-FRNLAN'))).toHaveLength(1);
        expect(commands.filter((cmd: string) => cmd.includes('wifi2.4-VPNLAN'))).toHaveLength(1);
        expect(commands.filter((cmd: string) => cmd.includes('wifi5-VPNLAN'))).toHaveLength(1);
    ...
    describe('Router Config Structure Validation', () => {
      it('should return valid RouterConfig structure', () => {
        ...
        expect(Array.isArray(result['/interface bridge port'])).toBe(true);
        expect(result['/interface bridge port'].every((cmd: string) => typeof cmd === 'string')).toBe(true);
        expect(result['/interface bridge port'].every((cmd: string) => cmd.startsWith('add bridge='))).toBe(true);
        validateRouterConfig(result, ['/interface bridge port']);
      });
      ...
      it('should generate commands in consistent format', () => {
        ...
        for (const command of commands) {
          ...
          expect(command).toMatch(/^add bridge=LANBridgeSplit interface=wifi(2\.4|5)-\w+LAN$/);
        }
        validateRouterConfig(result, ['/interface bridge port']);
      });
    });
  ...
  describe('CheckWireless', () => {
    ...
    it('should return true if SingleMode is present', () => {
      expect(CheckWireless({ SingleMode: {} as WirelessConfigType })).toBe(true);
    });
