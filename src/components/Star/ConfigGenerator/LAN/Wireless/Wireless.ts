...
export const SingleSSID = (SingleMode: WirelessConfig, WANLink: WANLink, DomesticLink: boolean): RouterConfig => {
    ...
    if(isWifi2_4) {
        ...
        config["/interface wifi"] = [...config["/interface wifi"], ...slaveConfig["/interface wifi"]];
    } else {
        // Otherwise, configure as master
        const masterConfig = Master(Network, "2.4", SingleMode);
        config["/interface wifi"] = [...config["/interface wifi"], ...masterConfig["/interface wifi"]];
    }
    ...
    if (bridgePortsConfig["/interface bridge port"]) {
        config["/interface bridge port"] = bridgePortsConfig["/interface bridge port"];
    }
    ...
    if (interfaceListConfig["/interface list member"]) {
        config["/interface list member"] = interfaceListConfig["/interface list member"];
    }
