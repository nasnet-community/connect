...
export function CheckMasters(WANLink: WANLink) {
    ...
    const isWifi2_4:boolean = Domestic?.includes("wifi2.4") || Foreign?.includes("wifi2.4");
    const isWifi5:boolean = Domestic?.includes("wifi5") || Foreign?.includes("wifi5");
    return {
        isWifi2_4,
        isWifi5
    }
...
export function StationMode(WANConfig: WANConfig, Link: "Domestic" | "Foreign"): RouterConfig {
    const config: RouterConfig = {
        "/interface wifi": [],
    };
...
export function Slave( Network: Networks, Band: Band, WirelessConfig: WirelessConfig ):RouterConfig {
    const config: RouterConfig = {
        "/interface wifi": []
    };
...
export function Master(Network: Networks, Band: Band, WirelessConfig: WirelessConfig):RouterConfig {
    const {SSID, Password, isHide, SplitBand} = WirelessConfig;
    const SSIDList = SSIDListGenerator(SSID, SplitBand);
