...
export const detectAvailableBands = (
...
export const DefaultBandToInterfaceName = (
    band: Band,
    availableBands: AvailableBands,
): string => {
    const totalBands =
        (availableBands.has2_4 ? 1 : 0) + availableBands.bands5GHz.length;
...
export const DisableInterfaces = (
    routerModels: RouterModels[],
): RouterConfig => {
    ...
    if (availableBands.has2_4) {
        ...
        config["/interface wifi"].push(
            `set [ find default-name=${defaultName} ] disabled=yes`,
        );
    }
...
export const CheckWANMaster = (
    WANLinks: WANLinks,
): { wifi2_4: boolean; wifi5: boolean } => {
    ...
    const hasWifi5 = interfaces.some(
        (iface) => iface.includes("wifi5") || iface.includes("wifi1"),
    );
...
export const CheckTrunkMaster = (
    routerModels: RouterModels[],
): { wifi2_4: boolean; wifi5: boolean } => {
    // Find the master router
    const masterRouter = routerModels.find(
        (router) => router.isMaster === true,
    );
    ...
    const isWifi24 =
        interfaceStr.includes("wifi2.4") || interfaceStr === "wifi2";
    const isWifi5 =
        interfaceStr.includes("wifi5") ||
        interfaceStr === "wifi1" ||
        interfaceStr.includes("wifi5-2");
...
export function WirelessBridge(
    WirelessConfig: WirelessConfig[],
    availableBands: AvailableBands,
): RouterConfig {
    const config: RouterConfig = {
    ...
    WirelessConfig.forEach((wireless) => {
        ...
        const hasCustomNetworkName =
            wireless.NetworkName && wireless.NetworkName !== baseNetwork;
...
export function WirelessSteering(
    wirelessConfig: WirelessConfig,
    availableBands: AvailableBands,
): RouterConfig {
    // Map WifiTarget to base network names
    ...
    // NetworkName should only be used if it's a custom name different from the base network
    const hasCustomNetworkName =
        wirelessConfig.NetworkName &&
        wirelessConfig.NetworkName !== baseNetwork;
...
export function WirelessSteeringAssignment(
