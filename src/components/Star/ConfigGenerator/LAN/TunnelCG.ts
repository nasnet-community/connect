...
export const generateAddressList = (address: string, listName: string, comment?: string): RouterConfig => {
    ...
    config["/ip firewall address-list"].push(`add ${addressListParams.join(' ')}`);
...
export const IPIPInterface = (ipip: IpipTunnelConfig): RouterConfig => {
    ...
    if (ipip.disabled !== undefined) interfaceParams.push(`disabled=${ipip.disabled ? 'yes' : 'no'}`);
    if (ipip.comment !== undefined) interfaceParams.push(`comment="${ipip.comment}"`);
    if (ipip.ipsecSecret !== undefined) interfaceParams.push(`ipsec-secret="${ipip.ipsecSecret}"`);
    if (ipip.keepalive !== undefined) interfaceParams.push(`keepalive=${ipip.keepalive}`);
    if (ipip.clampTcpMss !== undefined) interfaceParams.push(`clamp-tcp-mss=${ipip.clampTcpMss ? 'yes' : 'no'}`);
    if (ipip.dscp !== undefined) interfaceParams.push(`dscp=${ipip.dscp}`);
    if (ipip.dontFragment !== undefined) interfaceParams.push(`dont-fragment=${ipip.dontFragment}`);
    ...
    if (ipip.ipsecSecret && ipip.allowFastPath !== false) {
        ...
        interfaceParams.push(`allow-fast-path=${ipip.allowFastPath ? 'yes' : 'no'}`);
    }

    config["/interface ipip"].push(
...
export const EoipInterface = (eoip: EoipTunnelConfig): RouterConfig => {
    ...
    if (eoip.disabled !== undefined) interfaceParams.push(`disabled=${eoip.disabled ? 'yes' : 'no'}`);
    if (eoip.comment !== undefined) interfaceParams.push(`comment="${eoip.comment}"`);
    if (eoip.macAddress !== undefined) interfaceParams.push(`mac-address=${eoip.macAddress}`);
    if (eoip.ipsecSecret !== undefined) interfaceParams.push(`ipsec-secret="${eoip.ipsecSecret}"`);
    if (eoip.keepalive !== undefined) interfaceParams.push(`keepalive=${eoip.keepalive}`);
    if (eoip.arp !== undefined) interfaceParams.push(`arp=${eoip.arp}`);
    if (eoip.arpTimeout !== undefined) interfaceParams.push(`arp-timeout=${eoip.arpTimeout}`);
    if (eoip.clampTcpMss !== undefined) interfaceParams.push(`clamp-tcp-mss=${eoip.clampTcpMss ? 'yes' : 'no'}`);
    
    // Business logic: if ipsecSecret is used, allowFastPath must be false
    if (eoip.ipsecSecret && eoip.allowFastPath !== false) {
        interfaceParams.push(`allow-fast-path=no`);
    } else if (eoip.allowFastPath !== undefined) {
        interfaceParams.push(`allow-fast-path=${eoip.allowFastPath ? 'yes' : 'no'}`);
    }
    
    if (eoip.dontFragment !== undefined) interfaceParams.push(`dont-fragment=${eoip.dontFragment}`);
    if (eoip.dscp !== undefined) interfaceParams.push(`dscp=${eoip.dscp}`);
    if (eoip.loopProtect !== undefined) interfaceParams.push(`loop-protect=${eoip.loopProtect ? 'on' : 'off'}`);
    if (eoip.loopProtectDisableTime !== undefined) interfaceParams.push(`loop-protect-disable-time=${eoip.loopProtectDisableTime}`);
    if (eoip.loopProtectSendInterval !== undefined) interfaceParams.push(`loop-protect-send-interval=${eoip.loopProtectSendInterval}`);

    config["/interface eoip"].push(
        `add ${interfaceParams.join(' ')}`
    );
...
export const GreInterface = (gre: GreTunnelConfig): RouterConfig => {
    ...
    if (gre.disabled !== undefined) interfaceParams.push(`disabled=${gre.disabled ? 'yes' : 'no'}`);
    if (gre.comment !== undefined) interfaceParams.push(`comment="${gre.comment}"`);
    if (gre.ipsecSecret !== undefined) interfaceParams.push(`ipsec-secret="${gre.ipsecSecret}"`);
    if (gre.keepalive !== undefined) interfaceParams.push(`keepalive=${gre.keepalive}`);
    if (gre.clampTcpMss !== undefined) interfaceParams.push(`clamp-tcp-mss=${gre.clampTcpMss ? 'yes' : 'no'}`);
    if (gre.dscp !== undefined) interfaceParams.push(`dscp=${gre.dscp}`);
    if (gre.dontFragment !== undefined) interfaceParams.push(`dont-fragment=${gre.dontFragment}`);
...
export const VxlanInterface = (vxlan: VxlanInterfaceConfig): RouterConfig => {
    ...
    if (vxlan.disabled !== undefined) interfaceParams.push(`disabled=${vxlan.disabled ? 'yes' : 'no'}`);
    if (vxlan.comment !== undefined) interfaceParams.push(`comment="${vxlan.comment}"`);
    if (vxlan.port !== undefined) interfaceParams.push(`port=${vxlan.port}`);
    if (vxlan.hw !== undefined) interfaceParams.push(`hw=${vxlan.hw ? 'yes' : 'no'}`);
    if (vxlan.learning !== undefined) interfaceParams.push(`learning=${vxlan.learning ? 'yes' : 'no'}`);
    if (vxlan.allowFastPath !== undefined) interfaceParams.push(`allow-fast-path=${vxlan.allowFastPath ? 'yes' : 'no'}`);
    if (vxlan.arp !== undefined) interfaceParams.push(`arp=${vxlan.arp}`);
    if (vxlan.arpTimeout !== undefined) interfaceParams.push(`arp-timeout=${vxlan.arpTimeout}`);
    ...
    if (vxlan.bridgePVID !== undefined) interfaceParams.push(`bridge-pvid=${vxlan.bridgePVID}`);
    if (vxlan.checkSum !== undefined) interfaceParams.push(`checksum=${vxlan.checkSum ? 'yes' : 'no'}`);
    if (vxlan.dontFragment !== undefined) interfaceParams.push(`dont-fragment=${vxlan.dontFragment}`);
    if (vxlan.macAddress !== undefined) interfaceParams.push(`mac-address=${vxlan.macAddress}`);
    if (vxlan.maxFdbSize !== undefined) interfaceParams.push(`max-fdb-size=${vxlan.maxFdbSize}`);
    if (vxlan.ttl !== undefined) interfaceParams.push(`ttl=${vxlan.ttl}`);
    if (vxlan.vrf !== undefined) interfaceParams.push(`vrf=${vxlan.vrf}`);
    if (vxlan.vtepsIpVersion !== undefined) interfaceParams.push(`vteps-ip-version=${vxlan.vtepsIpVersion}`);
    ...
    if (vxlan.bumMode === 'unicast' && vtepsToAdd.length === 0) {
        throw new Error(`VXLAN ${vxlan.name} in unicast mode requires at least one VTEP configuration`);
    }
    ...
    if (tunnel.IPIP && tunnel.IPIP.length > 0) {
        tunnel.IPIP.forEach(ipip => {
            ...
            if (ipip.localAddress && ipip.remoteAddress) {
                ...
                const ipAddressConfig = generateIPAddress(tunnelIpAddress, ipip.name, `IP address for ${ipip.name} tunnel`);
                configs.push(ipAddressConfig);
                
                // Generate interface list membership (commonly used for LAN and VPN grouping)
                const interfaceListConfig = generateInterfaceList(ipip.name, ["LAN", "TUNNEL"]);
                ...
                const networkPrefix = ipip.localAddress.split('.').slice(0, 3).join('.') + '.0/30';
                const addressListConfig = generateAddressList(networkPrefix, "TUNNEL-NETWORKS", `Network for ${ipip.name}`);
    ...
    if (tunnel.Eoip && tunnel.Eoip.length > 0) {
        tunnel.Eoip.forEach(eoip => {
            ...
            if (eoip.localAddress && eoip.remoteAddress) {
                ...
                const ipAddressConfig = generateIPAddress(tunnelIpAddress, eoip.name, `IP address for ${eoip.name} tunnel`);
                configs.push(ipAddressConfig);
                
                // Generate interface list membership (EoIP is typically used for L2 extension)
                const interfaceListConfig = generateInterfaceList(eoip.name, ["LAN", "L2-TUNNEL"]);
                configs.push(interfaceListConfig);
                
                // Generate address list for tunnel endpoints
                const networkPrefix = eoip.localAddress.split('.').slice(0, 3).join('.') + '.0/30';
                const addressListConfig = generateAddressList(networkPrefix, "L2-TUNNEL-NETWORKS", `Network for ${eoip.name}`);
                configs.push(addressListConfig);
            }
        });
    }
    ...
    if (tunnel.Gre && tunnel.Gre.length > 0) {
        tunnel.Gre.forEach(gre => {
            ...
            if (gre.localAddress && gre.remoteAddress) {
                ...
                const ipAddressConfig = generateIPAddress(tunnelIpAddress, gre.name, `IP address for ${gre.name} tunnel`);
                configs.push(ipAddressConfig);
                
                // Generate interface list membership (GRE commonly used for routing)
                const interfaceListConfig = generateInterfaceList(gre.name, ["LAN", "GRE-TUNNEL"]);
                configs.push(interfaceListConfig);
                
                // Generate address list for tunnel network
                const networkPrefix = gre.localAddress.split('.').slice(0, 3).join('.') + '.0/30';
                const addressListConfig = generateAddressList(networkPrefix, "GRE-TUNNEL-NETWORKS", `Network for ${gre.name}`);
                configs.push(addressListConfig);
            }
        });
    }
    ...
    if (tunnel.Vxlan && tunnel.Vxlan.length > 0) {
        tunnel.Vxlan.forEach(vxlan => {
            ...
            if (vxlan.localAddress) {
                ...
                const ipAddressConfig = generateIPAddress(vxlanIpAddress, vxlan.name, `Gateway IP for VXLAN ${vxlan.name} (VNI ${vxlan.vni})`);
                configs.push(ipAddressConfig);
                
                // Generate interface list membership (VXLAN for L2 overlay networks)
                const interfaceListConfig = generateInterfaceList(vxlan.name, ["LAN", "VXLAN-OVERLAY"]);
                configs.push(interfaceListConfig);
                ...
                const addressListConfig = generateAddressList(vniNetwork, "VXLAN-NETWORKS", `VNI ${vxlan.vni} network for ${vxlan.name}`);
                configs.push(addressListConfig);
                ...
                if (vxlan.vteps && vxlan.vteps.length > 0) {
                    vxlan.vteps.forEach(vtep => {
                        if (vtep.remoteAddress) {
                            const vtepAddressConfig = generateAddressList(vtep.remoteAddress, "VXLAN-VTEPS", `VTEP peer for ${vxlan.name}`);
