...
export function addNetworkSegmentation(
  config: RouterConfig,
  vpnServer: VPNServer,
): void {
...
export const generateVPNFirewallRules = (
...
export const generateVPNAddressLists = (
...
export const generateVPNInterfaceLists = (
...
export function addCommonVPNConfiguration(
  config: RouterConfig,
  vpnServer: VPNServer,
): void {
  const { Users } = vpnServer;
...
export const isVPNProtocolConfigured = (
  state: StarState,
  protocol: string,
): boolean => {
  const vpnServer = state.LAN.VPNServer;
  ...
  switch (protocol.toLowerCase()) {
    case "wireguard":
      return !!(
        vpnServer.WireguardServers && vpnServer.WireguardServers.length > 0
      );
    case "openvpn":
...
export const getVPNUsersForProtocol = (
  state: StarState,
  protocol: string,
): Credentials[] => {
  const vpnServer = state.LAN.VPNServer;
