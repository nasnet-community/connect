...
export const ExportOpenVPN = (): RouterConfig => {
    ...
    const exportOpenVPNScriptContent: RouterConfig = {
        "": [
            ...
            "",
            "# Check if OpenVPN server is configured and enabled",
            ":local ovpnEnabled false;",
            ":local serverAddress \"\";",
            "",
            ":do {",
            "    :if ([/interface ovpn-server server print count-only] > 0) do={",
            "        :set ovpnEnabled true;",
            "        :log info \"OpenVPN server found\";",
            "    } else={",
            "        :log warning \"No OpenVPN server configured\";",
            "    }",
            "} on-error={",
            "    :log error \"Failed to check OpenVPN server status\";",
            "}",
            "",
            "# Setup MikroTik Cloud DDNS for server address",
            ":do {",
            "    /ip cloud set ddns-enabled=yes;",
            "    :delay 10s;",
            "    :set serverAddress [/ip cloud get dns-name];",
            "    :if ([:len $serverAddress] > 0) do={",
            "        :log info (\"Using Cloud DDNS: \" . $serverAddress);",
            "    } else={",
            "        :log warning \"Cloud DDNS not available - using router identity\";",
            "        :set serverAddress ([/system identity get name] . \".example.com\");",
            "    }",
            "} on-error={",
            "    :log warning \"Cloud DDNS setup failed\";",
            "    :set serverAddress ([/system identity get name] . \".example.com\");",
            "}",
            "",
            "# Check if certificate files exist (exported by ExportCert function)",
            ...
            "} else={",
            "    :log warning \"Certificate files missing - run ExportCert function first\";",
            "}",
            "",
            ...
            "        :foreach server in=[/interface ovpn-server server find] do={",
            "            /interface ovpn-server server set $server require-client-certificate=yes;",
            "        }",
            "        ",
            "        # Export configuration (RouterOS 7.17+ syntax with fallback)",
            "        :do {",
            "            :local serverName [/interface ovpn-server server get [find] name];",
            "            /interface ovpn-server server export-client-configuration server=$serverName server-address=$serverAddress ca-certificate=\"ca_certificate.crt\" client-certificate=\"client_bundle.crt\" client-cert-key=\"client_bundle.key\";",
            "        } on-error={",
            "            # Fallback for older RouterOS versions",
            "            /interface ovpn-server server export-client-configuration server-address=$serverAddress ca-certificate=\"ca_certificate.crt\" client-certificate=\"client_bundle.crt\" client-cert-key=\"client_bundle.key\";",
            "        }",
            "        ",
            "        :set exportSuccess true;",
            "        :log info \"OpenVPN client configuration exported successfully\";",
            "        ",
            "    } on-error={",
            "        :log error \"Failed to export OpenVPN configuration\";",
            "    }",
            "} else={",
            "    :log warning \"Prerequisites not met for export\";",
            "}",
            "",
            "# Summary",
            ":put \"\";",
            ":put \"=== OpenVPN Export Summary ===\";",
            ":put (\"Server Address: \" . $serverAddress);",
            "",
            "# OpenVPN Server Status", 
            ":if ($ovpnEnabled) do={",
            "    :put \"OpenVPN Server: ENABLED\";",
            "} else={",
            "    :put \"OpenVPN Server: NOT FOUND\";",
            "}",
            "",
            "# Certificate Status",
            ":if ($certsReady) do={",
            "    :put \"Certificates: READY\";",
            "} else={",
            "    :put \"Certificates: MISSING\";",
            "}",
            "",
            "# Export Status",
            ":if ($exportSuccess) do={",
            "    :put \"Export Status: SUCCESS\";",
            "} else={",
            "    :put \"Export Status: FAILED\";",
            "}",
            "",
            ":if ($exportSuccess) do={",
            "    :put \"Download the .ovpn file from Files menu and import it into your OpenVPN client\";",
            "} else={",
            "    :put \"Troubleshooting:\";",
            "    :if (!$ovpnEnabled) do={ :put \"- Configure OpenVPN server: /interface ovpn-server server set disabled=no\"; }",
            "    :if (!$certsReady) do={ :put \"- Run ExportCert function to export certificate files\"; }",
            "    :put \"- Check server status: /interface ovpn-server server print detail\";",
            "    :put \"- Check certificates: /certificate print detail\";",
            "}",
            "",
            ":log info \"=== OpenVPN Export Process Complete ===\";"
        ]
    };
