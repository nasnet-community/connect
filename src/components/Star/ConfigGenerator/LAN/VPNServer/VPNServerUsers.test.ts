...
describe("VPN Server Users Tests", () => {
  const testUsers: Credentials[] = [
    ...
    { Username: "user3", Password: "pass3", VPNType: ["SSTP", "IKeV2"] },
    {
      Username: "multiuser",
      Password: "passMulti",
      VPNType: ["Wireguard", "OpenVPN", "PPTP", "L2TP", "SSTP"],
    },
  ];
  ...
  describe("VPNServerCertificate", () => {
    it("should generate certificate configuration for VPN servers requiring certificates", () => {
      ...
      expect(
        comments.some((c: string) =>
          c.includes("Certificate configuration for VPN servers"),
        ),
      ).toBe(true);
      expect(
        comments.some((c: string) =>
          c.includes("SSTP Server requires certificates"),
        ),
      ).toBe(true);
      expect(
        comments.some((c: string) =>
          c.includes("OpenVPN Server requires certificates"),
        ),
      ).toBe(true);
    });
    ...
    it("should return appropriate message when no certificate-requiring VPN servers are configured", () => {
      ...
      expect(
        comments.some((c: string) =>
          c.includes("No VPN servers requiring certificates are configured"),
        ),
      ).toBe(true);
    });
  });
  ...
  describe("WireguardPeerAddress", () => {
    it("should generate WireGuard peer address update script", () => {
      ...
      expect(
        scriptCommands.some((cmd: string) => cmd.includes("name=WG-Update")),
      ).toBe(true);
    });
    ...
    it("should generate script with default parameters", () => {
      ...
      const scriptCommands = result["/system script"] || [];
      expect(
        scriptCommands.some((cmd: string) =>
          cmd.includes("WireGuard-Peer-Update"),
        ),
      ).toBe(true);
    });
  });
  ...
  describe("VPNServerBinding - Comprehensive Tests", () => {
    it("should generate static interface bindings for all supported VPN types", () => {
      ...
      testWithGenericOutput(
        ...
        () => {
          ...
          return {
            ...
            hasL2TPUser2: l2tpCommands.some((cmd) =>
              cmd.includes('user="user2"'),
            ),
            hasPPTPUser2: pptpCommands.some((cmd) =>
              cmd.includes('user="user2"'),
            ),
            hasSSTPUser3: sstpCommands.some((cmd) =>
              cmd.includes('user="user3"'),
            ),
            hasOVPNUser1: ovpnCommands.some((cmd) =>
              cmd.includes('user="user1"'),
            ),
      ...
      expect(l2tpCommands.some((cmd) => cmd.includes('user="user2"'))).toBe(
        true,
      );
      expect(l2tpCommands.some((cmd) => cmd.includes('user="multiuser"'))).toBe(
        true,
      );
      expect(pptpCommands.some((cmd) => cmd.includes('user="user2"'))).toBe(
        true,
      );
      expect(pptpCommands.some((cmd) => cmd.includes('user="multiuser"'))).toBe(
        true,
      );
      expect(sstpCommands.some((cmd) => cmd.includes('user="user3"'))).toBe(
        true,
      );
      expect(sstpCommands.some((cmd) => cmd.includes('user="multiuser"'))).toBe(
        true,
      );
      expect(ovpnCommands.some((cmd) => cmd.includes('user="user1"'))).toBe(
        true,
      );
      expect(ovpnCommands.some((cmd) => cmd.includes('user="multiuser"'))).toBe(
        true,
      );
    });
    ...
    it("should handle empty credentials array correctly", () => {
      ...
      const comments = result[""] || [];
      expect(comments.some((c) => c.includes("No credentials provided"))).toBe(
        true,
      );
    ...
    it("should handle users with only unsupported VPN types", () => {
      ...
      expect(
        comments.some((c) =>
          c.includes("No users configured for supported VPN binding types"),
        ),
      ).toBe(true);
    });
    ...
    it("should create interface list memberships for all created interfaces", () => {
      ...
      testWithGenericOutput(
        ...
        () => {
          const lanListEntries = listMemberCommands.filter((cmd) =>
            cmd.includes('list="LAN"'),
          );
          const vpnLanListEntries = listMemberCommands.filter((cmd) =>
            cmd.includes('list="VPN-LAN"'),
          );
          const l2tpInterface = listMemberCommands.filter((cmd) =>
            cmd.includes('interface="l2tp-l2tpuser"'),
          );
          const pptpInterface = listMemberCommands.filter((cmd) =>
            cmd.includes('interface="pptp-pptpuser"'),
          );
          ...
          return {
            ...
            isCorrect:
              listMemberCommands.length === 4 &&
              lanListEntries.length === 2 &&
              vpnLanListEntries.length === 2,
          };
        },
      );
    ...
    it("should include comprehensive documentation and summaries", () => {
      ...
      testWithGenericOutput(
        ...
        { testUsers },
        () => {
          const hasMainHeader = comments.some((c) =>
            c.includes("VPN Server Binding Configuration"),
          );
          const hasTotalUsers = comments.some((c) =>
            c.includes("Total users:"),
          );
          const hasSupportedTypes = comments.some((c) =>
            c.includes("Supported VPN types:"),
          );
          const hasBenefits = comments.some((c) =>
            c.includes("Static Interface Binding Benefits:"),
          );
          const hasSummary = comments.some((c) =>
            c.includes("VPN Server Binding Summary:"),
          );
          const hasInterfaceInfo = comments.some((c) =>
            c.includes("Interface List Memberships:"),
          );
      ...
      expect(
        comments.some((c) => c.includes("VPN Server Binding Configuration")),
      ).toBe(true);
      expect(comments.some((c) => c.includes("Total users:"))).toBe(true);
      expect(comments.some((c) => c.includes("Supported VPN types:"))).toBe(
        true,
      );
      expect(
        comments.some((c) => c.includes("Static Interface Binding Benefits:")),
      ).toBe(true);
      expect(
        comments.some((c) => c.includes("VPN Server Binding Summary:")),
      ).toBe(true);
      expect(
        comments.some((c) => c.includes("Interface List Memberships:")),
      ).toBe(true);
    });
  });
  ...
  describe("WireguardServerUsers", () => {
    it("should generate WireGuard peers for users", () => {
      ...
      expect(
        peerCommands.some((cmd: string) => cmd.includes('name="user1"')),
      ).toBe(true);
      expect(
        peerCommands.some((cmd: string) => cmd.includes('name="multiuser"')),
      ).toBe(true);
    });
    ...
    it("should handle server without interface address", () => {
      ...
      const peerCommands = result["/interface wireguard peers"] || [];
      expect(
        peerCommands.some((cmd: string) =>
          cmd.includes("Server interface address is required"),
        ),
      ).toBe(true);
    });
  });
  ...
  describe("OVPNServerUsers", () => {
    it("should generate OpenVPN users", () => {
      ...
      expect(
        secretCommands.some(
          (cmd: string) =>
            cmd.includes('name="user1"') && cmd.includes("service=ovpn"),
        ),
      ).toBe(true);
      expect(
        secretCommands.some(
          (cmd: string) =>
            cmd.includes('name="multiuser"') && cmd.includes("service=ovpn"),
        ),
      ).toBe(true);
    });
    ...
    it("should handle no OpenVPN users", () => {
      ...
      const secretCommands = result["/ppp secret"] || [];
      expect(
        secretCommands.some((cmd: string) =>
  ...
  describe("PptpServerUsers", () => {
    it("should generate PPTP users", () => {
      ...
      expect(
        secretCommands.some(
          (cmd: string) =>
            cmd.includes('name="user2"') && cmd.includes("service=pptp"),
        ),
      ).toBe(true);
      expect(
        secretCommands.some(
          (cmd: string) =>
            cmd.includes('name="multiuser"') && cmd.includes("service=pptp"),
        ),
      ).toBe(true);
    });
  });
  ...
  describe("L2tpServerUsers", () => {
    it("should generate L2TP users", () => {
      ...
      expect(
        secretCommands.some(
          (cmd: string) =>
            cmd.includes('name="user2"') && cmd.includes("service=l2tp"),
        ),
      ).toBe(true);
      expect(
        secretCommands.some(
          (cmd: string) =>
            cmd.includes('name="multiuser"') && cmd.includes("service=l2tp"),
        ),
      ).toBe(true);
    });
  });
  ...
  describe("SstpServerUsers", () => {
    it("should generate SSTP users", () => {
      ...
      expect(
        secretCommands.some(
          (cmd: string) =>
            cmd.includes('name="user3"') && cmd.includes("service=sstp"),
        ),
      ).toBe(true);
      expect(
        secretCommands.some(
          (cmd: string) =>
            cmd.includes('name="multiuser"') && cmd.includes("service=sstp"),
        ),
      ).toBe(true);
    });
  });
  ...
  describe("Ikev2ServerUsers", () => {
    it("should generate IKEv2 users info", () => {
      ...
      expect(
        comments.some((c: string) =>
          c.includes("IKEv2 users are managed through IPsec identities"),
        ),
      ).toBe(true);
      expect(
        comments.some((c: string) =>
          c.includes("1 users configured for IKEv2"),
        ),
      ).toBe(true);
    });
  });
  ...
  describe("VPNServerUsersWrapper", () => {
    it("should generate complete VPN users configuration", () => {
      ...
      expect(
        comments.some((c: string) =>
          c.includes("VPN Server Users Configuration Summary"),
        ),
      ).toBe(true);
      expect(comments.some((c: string) => c.includes("Total users: 4"))).toBe(
        true,
      );
    });
    ...
    it("should handle empty users array", () => {
      ...
      expect(
        comments.some((c: string) =>
          c.includes("No VPN types found in user credentials"),
        ),
      ).toBe(true);
    });
  });
  ...
  describe("Edge Cases", () => {
    it("should handle mixed VPN type filtering correctly", () => {
      ...
      expect(
        pptpCommands.filter((cmd: string) => cmd.includes("add")).length,
      ).toBe(2);
      expect(
        pptpCommands.some((cmd: string) => cmd.includes('name="pptp-only"')),
      ).toBe(true);
      expect(
        pptpCommands.some((cmd: string) => cmd.includes('name="multi-vpn"')),
      ).toBe(true);
    });
...
describe("VPNServerUsers Module Tests - Real Context Data", () => {
  ...
  describe("VPNServerBinding with User Context", () => {
    it("should generate VPN server bindings for the user context", () => {
      ...
      console.log(
        `  Expected binding names: ${expectedBindingNames.join(", ")}`,
      );
    ...
    it("should filter users correctly by supported VPN types", () => {
      ...
      console.log(
        `  Has "unsupported types" comment: ${hasUnsupportedComment}`,
      );
