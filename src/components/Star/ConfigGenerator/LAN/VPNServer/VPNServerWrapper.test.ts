...
describe("VPN Server Wrapper Tests", () => {
  ...
  describe("VPNServerWrapper - Main Function", () => {
    ...
    it("should handle empty VPN server configuration", () => {
      ...
      const comments = result[""] || [];
      expect(
        comments.some((c: string) => c.includes("No VPN users configured")),
      ).toBe(true);
    });
    ...
    it("should handle VPN server with users but no server configurations", () => {
      ...
      expect(
        comments.some((c: string) => c.includes("Configured Protocols:")),
      ).toBe(true);
  ...
  describe("L2tpServerWrapper", () => {
    it("should generate complete L2TP server configuration with users", () => {
      ...
      expect(comments.some((c: string) => c.includes("IPsec: required"))).toBe(
        true,
      );
  ...
  describe("SstpServerWrapper", () => {
    it("should generate complete SSTP server configuration with users", () => {
      ...
      expect(
        comments.some((c: string) => c.includes("Certificate: server-cert")),
      ).toBe(true);
  ...
  describe("Ikev2ServerWrapper", () => {
    it("should generate complete IKEv2 server configuration with users", () => {
      ...
      expect(
        comments.some((c: string) => c.includes("Auth Method: pre-shared-key")),
      ).toBe(true);
  ...
  describe("Edge Cases and Error Handling", () => {
    ...
    it("should handle disabled VPN servers", () => {
      ...
      const comments = result[""] || [];
      expect(comments.some((c: string) => c.includes("PPTP: 1 users"))).toBe(
        true,
      );
      expect(comments.some((c: string) => c.includes("L2TP: 1 users"))).toBe(
        true,
      );
    });
  });
  ...
  describe("Network Configuration", () => {
    it("should verify network assignments in summary", () => {
      ...
      const comments = result[""] || [];
      expect(
        comments.some((c: string) => c.includes("WireGuard: 192.168.170.0/24")),
      ).toBe(true);
      expect(
        comments.some((c: string) => c.includes("OpenVPN: 192.168.60.0/24")),
      ).toBe(true);
    });
