...
describe("Networks", () => {
  ...
  it("builds a complete routed network config when mangle generation is enabled", () => {
    const result = NetworkBaseGenerator("Foreign", "192.168.30.0/24", undefined, true);

    expect(result["/interface bridge"][0]).toContain('name="LANBridgeForeign"');
    expect(result["/ip dhcp-server"][0]).toContain('name="DHCP-Foreign"');
    expect(result["/routing table"][0]).toContain('name="to-Foreign"');
    expect(result["/ip firewall mangle"][0]).toContain('new-routing-mark="to-Foreign"');
  });
  ...
  it("omits domestic mangle rules when skipMangle is requested", () => {
    ...
    expect(result["/interface bridge"][0]).toContain('name="LANBridgeDomestic"');
  ...
  it("generates additional named networks with prefixed bridge names and routing marks", () => {
    ...
    expect(result["/interface bridge"][0]).toContain('name="LANBridgeForeign-Guest"');
    expect(result["/ip firewall mangle"][0]).toContain('new-routing-mark="to-Foreign-Guest"');
  });
