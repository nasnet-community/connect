...
import type { OccupiedInterface } from "../StarContext/ChooseType";
import {
...
export interface UseInterfaceManagementReturn {
  ...
  getInterfaceUsageInfo$: QRL<
    (interfaceName: string) => Promise<string | null>
  >;
...
export function useInterfaceManagement(): UseInterfaceManagementReturn {
  ...
  const markInterfaceAsOccupied$ = $(
    async (
      ...
      const updatedRouterModels = starContext.state.Choose.RouterModels.map(
        (rm) =>
          rm.isMaster
            ? {
                ...rm,
   ...
   */
  const isInterfaceAvailable$ = $(
    async (interfaceName: string): Promise<boolean> => {
      const masterOccupied = getMasterOccupiedInterfaces(
   ...
   * Only checks the master router's occupied interfaces
   */
  const getInterfaceUsageInfo$ = $(
    async (interfaceName: string): Promise<string | null> => {
      const masterOccupied = getMasterOccupiedInterfaces(
        starContext.state.Choose.RouterModels,
      );
   ...
   */
  const getAllOccupiedInterfacesList$ = $(
    async (): Promise<OccupiedInterface[]> => {
   ...
   */
  const updateInterfaceOccupation$ = $(
    async (
      oldInterface: InterfaceType | null,
      newInterface: InterfaceType | null,
      usedBy: "Trunk" | "WAN" | "VPN" | "LAN" | "Other",
